...
bool isSequentialPlugin(CoreIR::Wireable* fst, PluginMap& pluginMap) {
  if (!isInstance(fst)) { return false; }
...
Wireable* extractSource(Select* sel) {
  ...
  if (fromSelf(sel) && (!isSelect(p))) { return sel; }

  // Base case for non self connections.
  if (!isSelect(p)) { return p; }

  return extractSource(toSelect(p));
}

WireNode getNode(const NGraph& g, const vdisc vd) { return g.getNode(vd); }

Conn getConn(const NGraph& g, const edisc ed) { return g.getConn(ed); }
...
std::vector<Conn> NGraph::getInputConnections(const vdisc vd) const {
  ...
  for (auto out_edge_desc : inEdges(vd)) {
    ...
    if (hasLabel(out_edge_desc)) {
      Conn edge_conn = getConn(out_edge_desc);
...
std::vector<Wireable*> NGraph::getInputs(const vdisc vd) const {
  ...
  for (auto in_edge_desc : inEdges(vd)) {
    ...
    Conn edge_conn = getConn(in_edge_desc);
...
vector<vdisc> NGraph::vertsWithNoIncomingEdge() const {
  ...
  for (auto v : getVerts()) {
    ...
    if (inEdges(v).size() == 0) { vs.push_back(v); }
  }

  return vs;
}

int numVertices(const NGraph& g) { return g.numVertices(); }
  ...
  while (nodes.size() > 0) {
    ...
    for (auto& node : nodes) {
      ...
      if (inEds.size() > 0) {
        ...
        if (allInputsInPriorLevel) { level.push_back(node); }
      }
    }
  ...
  if (!(topo_order.size() == (uint)numVertices(g))) {
    ...
    for (auto vert : g.getVerts()) {
      if (!elem(vert, topo_order)) {
        cout << "\tNot in topological sort: " << vert << ", "
             << g.getNode(vert).getWire()->toString() << ", "
             << g.getNode(vert).getWire()->getType()->toString() << endl;
        cout << "\tOUTPUT CONNECTIONS" << endl;
        for (auto ed : g.outEdges(vert)) {
          Conn c = g.getConn(ed);
          cout << "\t\t" << c.first.getWire()->toString() << " <---> "
               << c.second.getWire()->toString() << endl;
        }
        ...
        for (auto ed : g.inEdges(vert)) {
          ...
          cout << "\t\t" << c.first.getWire()->toString() << " <---> "
               << c.second.getWire()->toString() << endl;
          // cout << "\t\t" << nodeString(g.getNode(c.first)) << " <---> " <<
          // nodeString(g.getNode(c.second)) << endl;
        }
      }
    }
...
void addConnection(
  ...
  if (isSequential(p1, pluginMap)) { c1_disc_it = imap.find(outputNode(p1)); }
...
void buildOrderedGraph(PluginMap& pluginMap, Module* mod, NGraph& g) {
  ...
  for (Conn conn : ord_conns) { addConnection(pluginMap, imap, conn, g); }
}
...
void setEdgeClean(const edisc ed, NGraph& g) {
  Conn c = g.getConn(ed);
...
bool inputsAreClean(const vdisc vd, const NGraph& g) {
  for (auto& conn : g.getInputConnections(vd)) {
    if (conn.first.needsMask()) { return false; }
  }

  return true;
}

void eliminateMasks(const std::deque<vdisc>& topoOrder, NGraph& g) {
...
bool isThreadShared(const vdisc v, const NGraph& g) {
  ...
  for (auto& conn : g.outEdges(v)) {
    ...
    if (g.getNode(dest).getThreadNo() != threadNo) { return true; }
  }
  ...
  for (auto& vd : g.getVerts()) {
    ...
    if (isGraphInput(w)) {
      ...
      if (tp->getKind() == CoreIR::Type::TK_Named) {
        ...
        if (ntp->toString() == "coreir.clk") { clockInputs.push_back(inSel); }
      }
    }
  }

  if (clockInputs.size() > 1) {
    cout << "ERROR: Circuit has " << clockInputs.size()
         << " clocks, but this simulator currently supports only one" << endl;
    cout << "The clocks are " << endl;
    for (auto& clk : clockInputs) { cout << clk->toString() << endl; }
    assert(false);
  }

  if (clockInputs.size() == 0) { return nullptr; }
...
bool isSubgraphInput(
  ...
  for (auto ec : g.inEdges(vd)) {
    for (auto& other : nodes) {
      if (g.source(ec) == other) { return false; }
    }
  }
...
bool subgraphHasAllSequentialOutputs(
  ...
  for (auto& vd : nodes) {
    if (isSubgraphOutput(vd, nodes, g)) {
      WireNode wd = g.getNode(vd);
      if (!wd.isSequential) { return false; }
    }
  }
...
bool subgraphHasSequentialInput(
  ...
  for (auto& vd : nodes) {
    if (isSubgraphInput(vd, nodes, g)) {
      ...
      WireNode wd = g.getNode(vd);
      if (wd.isSequential) { return true; }
    }
  }
...
bool isConstant(const WireNode& wd) {
  ...
  if (isInstance(w)) {
    ...
    return (name == "coreir.const") || (name == "corebit.const");
  }
