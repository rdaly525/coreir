// Blatently taken from LLVM TODO how do I explicitly give credit to them?
...
// License. See license/LLVM_LICENSE.TXT for details.
//
//===----------------------------------------------------------------------===//
//
...
template <typename From> struct simplify_type<std::shared_ptr<From>> {
  ...
  static SimpleType& getSimplifiedValue(std::shared_ptr<From>& Val) {
    return *Val;
  }
};
...
template <typename To, typename From> struct isa_impl<
  To,
  From,
...
template <class To, class FromTy> struct cast_convert_val<To, FromTy, FromTy> {
  ...
  static typename cast_retty<To, FromTy>::ret_type doit(const FromTy& Val) {
    typename cast_retty<To, FromTy>::ret_type
      Res2 = (typename cast_retty<To, FromTy>::ret_type) const_cast<FromTy&>(
        Val);
    return Res2;
  }
};
...
template <class X> struct is_simple_type {
  static const bool
    value = std::is_same<X, typename simplify_type<X>::SimpleType>::value;
};
...
//
template <class X, class Y> inline typename std::enable_if<
