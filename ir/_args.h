...
template <typename T> Const* Const_impl(T val);

template <> Const* Const_impl<bool>(bool val);
template <> Const* Const_impl<int>(int val);
template <> Const* Const_impl<std::string>(std::string val);
template <> Const* Const_impl<Type*>(Type* val);

template <typename T>
typename std::enable_if<std::is_same<T, bool>::value, ArgPtr>::type Const(
  T val) {
  return Const_impl<bool>(val);
}

template <typename T> inline typename std::enable_if<
  !std::is_same<T, bool>::value && std::is_convertible<T, int>::value,
  ArgPtr>::type
Const(T val) {
  return Const_impl<int>(val);
}

template <typename T> inline typename std::
  enable_if<std::is_convertible<T, std::string>::value, ArgPtr>::type
  Const(T val) {
  return Const_impl<std::string>(val);
}

template <typename T> inline
  typename std::enable_if<std::is_convertible<T, Type*>::value, ArgPtr>::type
  Const(T val) {
