...
bool isBitType(const Type& tp) {
  if (isa<BitType>(tp)) { return true; }

  if (isa<BitInType>(tp)) { return true; }

  if (isa<BitInOutType>(tp)) { return true; }
...
std::vector<Connection> getReceiverConnections(CoreIR::Wireable* w) {
  ...
  for (auto sel : w->getConnectedWireables()) {
    if (sel->getType()->getDir() == Type::DK_In) { conns.push_back({sel, w}); }
  }
...
std::vector<Connection> getSourceConnections(CoreIR::Wireable* w) {
  ...
  for (auto sel : w->getConnectedWireables()) {
    if (sel->getType()->getDir() == Type::DK_Out) { conns.push_back({sel, w}); }
  }
...
bool isAncestorOf(Wireable* const possibleAncestor, Wireable* const w) {
  if (possibleAncestor == w) { return true; }
...
CoreIR::Select* getDriverSelect(CoreIR::Select* const src) {
  ...
  if (connected.size() == 0) {
    ...
    if (parentDriver == nullptr) { return nullptr; }
...
maybe<BitVector> getSignalBitVec(const std::vector<CoreIR::Select*>& signals) {
  ...
  for (uint i = 0; i < ((uint)bv.bitLength()); i++) {
    ...
    if (sigi == nullptr) { return maybe<BitVector>(); }

    // cout << "sigi = " << sigi->toString() << endl;

    Wireable* src = extractSource(sigi);

    if (!isConstant(src)) { return maybe<BitVector>(); }
...
std::vector<Connection> unpackConnection(const CoreIR::Connection& conn) {
  ...
  if (isBitType(*fstType)) { return {conn}; }

  if (fstType->getKind() == Type::TK_Named) { return {conn}; }
...
void portToConstant(
  ...
  def->connect(wbPassthrough->sel("in"), replacement);
...
void setRegisterInit(
  ...
  if (!contains_key(instanceName, def->getInstances())) { return; }
  ...
  def->connect(pt->sel("in"), replacement);
