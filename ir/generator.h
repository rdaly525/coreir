...
class Generator : public GlobalValue, public VerilogPrimitive {
 ...
 public:
  ...
  // If the typegen can create a type, it will verify types match, else it will
  // just use the type
  ...
  std::pair<Params, Values> getModParams(Values genargs) {
    if (modParamsGen) { return modParamsGen(getContext(), genargs); }
    else {
