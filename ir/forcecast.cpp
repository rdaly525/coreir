...
// Cast to bool
template <> Value* TemplatedConst<bool>::forceCast(ValueType* vt) const {
...
template <> Value* TemplatedConst<int>::forceCast(ValueType* vt) const {
  Context* c = vt->getContext();
  switch (vt->getKind()) {
  case ValueType::VTK_Bool: {
...
template <> Value* TemplatedConst<BitVector>::forceCast(ValueType* vt) const {
  Context* c = vt->getContext();
  switch (vt->getKind()) {
  case ValueType::VTK_Bool: {
...
template <> Value* TemplatedConst<std::string>::forceCast(ValueType* vt) const {
  ASSERT(0, "NYI, Casting strings");
  return nullptr;
}

template <> Value* TemplatedConst<Type*>::forceCast(ValueType* vt) const {
  ASSERT(0, "Cannot cast a Type*");
  return nullptr;
}

template <> Value* TemplatedConst<Module*>::forceCast(ValueType* vt) const {
  ASSERT(0, "Cannot cast a Module");
  return nullptr;
}

template <> Value* TemplatedConst<Json>::forceCast(ValueType* vt) const {
  Context* c = vt->getContext();
  switch (vt->getKind()) {
  case ValueType::VTK_Bool: {
...
template <> bool TemplatedConst<bool>::canCast(ValueType* vt) const {
  return isa<BoolType>(vt) || isa<IntType>(vt) || isa<BitVectorType>(vt) ||
    isa<JsonType>(vt);
}
template <> bool TemplatedConst<int>::canCast(ValueType* vt) const {
  return isa<BoolType>(vt) || isa<IntType>(vt) || isa<BitVectorType>(vt) ||
    isa<JsonType>(vt);
}
template <> bool TemplatedConst<BitVector>::canCast(ValueType* vt) const {
  return isa<BoolType>(vt) || isa<IntType>(vt);
}

template <> bool TemplatedConst<std::string>::canCast(ValueType* vt) const {
  return false;
}

template <> bool TemplatedConst<CoreIR::Type*>::canCast(ValueType* vt) const {
  return false;
}

template <> bool TemplatedConst<Module*>::canCast(ValueType* vt) const {
  return false;
}

template <> bool TemplatedConst<Json>::canCast(ValueType* vt) const {
  return isa<BoolType>(vt) || isa<IntType>(vt) || isa<StringType>(vt) ||
    isa<JsonType>(vt);
}
