...
template <typename T> int num_elems(std::vector<std::vector<T>>& v) {
  int i = 0;
  for (auto t : v) { i += t.size(); }
  return i;
}

template <typename I> std::vector<unsigned> inds(const std::vector<I>& elems) {
  std::vector<unsigned> is(elems.size());
  std::iota(begin(is), end(is), 0);
  return is;
}

template <typename T> void concat(T& extend, const T& add) {
  extend.insert(end(extend), begin(add), end(add));
}

template <typename E, typename T> void remove(E e, T& t) {
  t.erase(std::remove(begin(t), end(t), e), end(t));
}

template <typename E, typename T> bool elem(E e, std::vector<T>& t) {
  return std::find(begin(t), end(t), e) != end(t);
}

template <typename T> bool elem(T e, const std::unordered_set<T>& t) {
  return t.find(e) != end(t);
}

template <typename T> bool elem(T e, const std::deque<T>& t) {
  return std::find(begin(t), end(t), e) != end(t);
}

template <typename T> bool elem(T e, const std::set<T>& t) {
  return t.find(e) != end(t);
}
...
template <typename I, typename F> void delete_if(I& c, F f) {
  c.erase(remove_if(begin(c), end(c), f), end(c));
}

template <typename T> void subtract(T& a, const T& b) {
  delete_if(a, [&b](const typename T::value_type& i) {
    return std::find(begin(b), end(b), i) != end(b);
  });
}

template <typename T, typename Q> std::pair<T, Q> mk_pair(T t, Q q) {
  return std::pair<T, Q>(t, q);
}

// TODO: Figure out why this implementation always calls
// the default constructor
template <typename InputIt1, typename InputIt2, typename OutputIt>
OutputIt zip(InputIt1 ps, InputIt1 pe, InputIt2 qs, OutputIt r) {
  while (ps != pe) {
    *r = mk_pair(*ps, *qs);
    ++ps;
    ++qs;
...
template <typename T, typename F> void drop_while(T& t, F f) {
...
template <typename T, typename F> void take_while(T& t, F f) {
...
std::vector<T> select(const std::vector<T>& v, F f) {
  ...
  for (auto e : v) {
    if (f(e)) { selected.push_back(e); }
  }
  return selected;
}

template <typename I, typename P> std::vector<unsigned> select_neighbors(
...
std::vector<std::vector<unsigned>> connected_components_by(
  ...
  while (inds.size() > 0) { components.push_back(dfs_by(inds, elems, p)); }
...
std::vector<std::vector<I>> connected_components_by_elems(
  ...
  for (auto cc : ccs) {
    ...
    for (auto i : cc) { cc_elems.push_back(elems[i]); }
...
template <typename I, typename F>
std::vector<I> greedy_chain(const I& init, const std::vector<I>& elems, F f) {
  std::vector<I> chain;
...
// Start intersection
template <typename A>
std::set<A> intersection(const std::set<A>& l, const std::set<A>& r) {
  std::set<A> it;
  set_intersection(
    std::begin(l),
    std::end(l),
...
template <typename A>
std::deque<A> intersection(const std::deque<A>& l, const std::deque<A>& r) {
  std::deque<A> it;
  for (auto& e : l) {
    if (elem(e, r)) { it.push_back(e); }
  }
  return it;
}

template <typename A>
std::vector<A> intersection(const std::vector<A>& l, const std::vector<A>& r) {
  std::vector<A> it;
  for (auto& e : l) {
    if (elem(e, r)) { it.push_back(e); }
  }
...
template <typename T>
std::deque<T> difference(const std::deque<T>& a, const std::deque<T>& b) {
  std::deque<T> diff;
  for (auto& e : a) {
    if (!elem(e, b)) { diff.push_back(e); }
  }
...
std::vector<I> concat_all(const std::vector<std::vector<I>>& vs) {
  std::vector<I> all_vs;
  for (auto v : vs) { concat(all_vs, v); }
  return all_vs;
}

template <typename I> std::vector<I> sort_unique(const std::vector<I>& elems) {
  auto elems_cp = elems;
...
B map_find(const A& a, const std::unordered_map<A, B>& m) {
  auto f = m.find(a);
  if (f == std::end(m)) { DBG_ASSERT(false); }
...
template <typename T, typename F>
std::vector<T> partial_order_maxima(const std::vector<T>& elems, F f) {
  std::vector<T> maxima;
  for (unsigned i = 0; i < elems.size(); i++) {
    auto e = elems[i];
    bool gt_all = true;
    for (unsigned j = 0; j < elems.size(); j++) {
      auto a = elems[j];
      if (i != j && !f(a, e)) { gt_all = false; }
    }
    if (gt_all) { maxima.push_back(e); }
  }
  return maxima;
}

template <typename Elem, typename Orthogonal> Elem next_orthogonal_to_all(
  const std::vector<Elem>& to_check,
  ...
  for (unsigned i = 0; i < to_return_from.size(); i++) {
    ...
    if (all_of(begin(to_check), end(to_check), [q, orthogonal](const Elem& r) {
...
template <typename Elem, typename Orthogonal> std::vector<Elem> take_basis(
...
template <typename T> std::vector<T*> ptrs(std::vector<T>& elems) {
  std::vector<T*> pointers;
  for (T& e : elems) { pointers.push_back(&e); }
  return pointers;
}
...
template <typename T>
bool elems_equal(const std::vector<T>& x, const std::vector<T>& y) {
...
template <typename A, typename F> bool any_of(const A& container, F f) {
  return any_of(begin(container), end(container), f);
}

template <typename A, typename F> bool all_of(const A& container, F f) {
