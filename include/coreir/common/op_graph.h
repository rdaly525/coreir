...
class InstanceValue {
 ...
 public:
  ...
  void setNeedsMask(const bool val) { mustMask = val; }
};
...
template <typename Node, typename Edge> class DirectedGraph {
 ...
 public:
  ...
  std::vector<edisc> getEdges() const { return edges; }

  std::vector<vdisc> getVerts() const { return verts; }
  ...
  void addEdgeLabel(const edisc ed, const Conn& conn) { edgeNames[ed] = conn; }

  edisc nextEdgeDisc() const { return edges.size(); }

  vdisc nextVertexDisc() const { return verts.size(); }
...
std::vector<vdisc> vertsWithNoIncomingEdge(const DirectedGraph<Node, Edge>& g) {
...
class NGraph {
 ...
 public:
  WireNode getNode(const vdisc vd) const { return g.getNode(vd); }

  Conn getConn(const edisc ed) const { return g.getConn(ed); }

  bool hasLabel(const edisc ed) const { return g.hasLabel(ed); }

  vdisc source(const edisc ed) const { return g.source(ed); }
  ...
  vdisc target(const edisc ed) const { return g.target(ed); }

  edisc addEdge(const vdisc s, const vdisc e) { return g.addEdge(s, e); }

  edisc nextEdgeDisc() const { return g.nextEdgeDisc(); }

  vdisc nextVertexDisc() const { return g.nextVertexDisc(); }
  ...
  void addVertex(const vdisc v) { g.addVertex(v); }

  vdisc addVertex() { return g.addVertex(); }

  std::vector<edisc> outEdges(const vdisc vd) const { return g.outEdges(vd); }

  std::vector<edisc> inEdges(const vdisc vd) const { return g.inEdges(vd); }

  std::vector<edisc> getEdges() const { return g.getEdges(); }

  std::vector<vdisc> getVerts() const { return g.getVerts(); }

  bool containsOpNode(CoreIR::Wireable* w) const {
    WireNode wn = combNode(w);

    for (auto& vpair : g.getVertNames()) {
      if (vpair.second == wn) { return true; }
    }
  ...
  vdisc getOpNodeDisc(CoreIR::Wireable* w) const {
    ...
    for (auto& vpair : g.getVertNames()) {
      if (vpair.second == wn) { return vpair.first; }
    }
    coreir_unreachable();
  }
