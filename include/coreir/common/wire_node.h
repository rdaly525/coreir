...
class WireNode {
 ...
 public:
  ...
  void setThreadNo(const int i) { threadNumber = i; }
  ...
  bool isOpNode() const {
    ...
    if (fromSelf(sel) && (!isSelect(p))) { return true; }
  ...
  bool operator==(const WireNode& other) const {
    return (wire == other.wire) && (isSequential == other.isSequential) &&
      (isReceiver == other.isReceiver);
  }

  std::string toString() const {
    return getWire()->toString() + ", sequential ? " +
      std::to_string(isSequential) + ", isReceiver ? " +
      std::to_string(isReceiver);
  }
};
...
static inline std::string nodeString(const WireNode& w) {
  if (w.isSequential) {
    if (w.isReceiver) { return w.getWire()->toString() + ", receiver"; }
    else {
...
template <> struct hash<CoreIR::WireNode> {
  std::size_t operator()(const CoreIR::WireNode& k) const {
    ...
    return (
      (hash<CoreIR::Wireable*>()(k.getWire())) ^
      (hash<bool>()(k.isSequential) << 1) ^ (hash<bool>()(k.isReceiver) << 2) ^
