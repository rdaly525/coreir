...
class CreateCombView : public InstanceGraphPass {
 ...
 public:
  ...
  bool hasComb(Module* m) {
    return (combs.count(m) > 0) &&
      (combs[m].inputs.size() > 0 || combs[m].outputs.size() > 0);
  }

  std::set<SelectPath>& getSrc(Module* m) { return srcs[m]; }
  std::set<SelectPath>& getSnk(Module* m) { return snks[m]; }
  Comb& getComb(Module* m) { return combs[m]; }
 ...
 private:
  ...
  void traverseOut2In(
    Wireable* curin,
    Wireable* out,
    std::map<Wireable*, Output*>& outputInfo,
    std::map<Wireable*, Input*>& inputInfo);
};
