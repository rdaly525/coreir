...
static inline std::string maskResultStr(const uint w, const std::string& expr) {
...
static inline std::string ite(
  const std::string& condition,
  const std::string& trueRes,
  const std::string& falseRes) {
  std::string cntrlMask = parens(
                            parens(
                              parens(parens("int") + condition) + " << 31") +
                            " >> 31") +
    " | ";
...
static inline std::string ln(const std::string& s) { return s + ";\n"; }
