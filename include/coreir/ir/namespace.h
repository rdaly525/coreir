...
class Namespace {
 ...
 public:
  ...
  const std::map<std::string, Generator*>& getGenerators() {
  ...
  // TODO depreciate the following function from namespace. Should only be
  // created from a static make function
  ...
  bool hasGenerator(std::string iname) {
    return generatorList.count(iname) > 0;
  }
  bool hasGlobalValue(std::string iname) {
    return moduleList.count(iname) > 0 || generatorList.count(iname) > 0;
  }
