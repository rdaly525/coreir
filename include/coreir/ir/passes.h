...
class InstanceVisitorPass : public Pass {
 public:
  ...
  static bool classof(const Pass* p) {
    return p->getKind() == PK_InstanceVisitor;
  }
  virtual void initialize(int argc, char** argv) override {}
  virtual void releaseMemory() override {}
  virtual void setAnalysisInfo() override {}
...
// Loops through the InstanceDAG from bottom up. Instane DAG is analogous to
// CallGraph in LLVM. If the Instance is linked in from a different namespace or
...
class InstanceGraphPass : public Pass {
 ...
 public:
  ...
  static bool classof(const Pass* p) {
    return p->getKind() == PK_InstanceGraph;
  }
  virtual void initialize(int argc, char** argv) override {}
