...
class ModuleDef {
 ...
 public:
  ModuleDef(Module* m);
  ~ModuleDef();
  const std::map<std::string, Instance*>& getInstances(void) const {
    return instances;
  }

  std::map<std::string, Wireable*> getWireables(bool include_self) const;
  ...
  ////Note these will invalidate iterators from the following: getInstances(),
  /// getConnections(), getConnectedWireables()
