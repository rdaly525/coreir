...
def test_inline_example(rst):
    ...
    res = delegator.run(f"bazel-bin/binary/coreir -i {in_file} -l commonlib -p \"rungenerators; inline_single_instances\" -o {res_file}")
...
def test_clock_gate(rst, suffix):
    ...
    res = delegator.run(f"bazel-bin/binary/coreir -i {in_file} -l commonlib -p \"rungenerators; inline_single_instances; clock_gate; inline_single_instances\" -o {res_file}")
    assert not res.return_code, res.out + res.err
    res = delegator.run(f"diff {res_file} {gold_file}")
    assert not res.return_code, res.out + res.err
