...
vector<vector<vdisc>> groupByImplName(
  const vector<vector<vdisc>>& identicalComps,
  ...
  for (uint i = 0; i < identicalComps[0].size(); i++) {
    vector<vdisc> gp;
    for (int j = 0; j < width; j++) { gp.push_back(identicalComps[j][i]); }
    groups.push_back(gp);
  }
...
bool splitNodeEdgesCorrect(const NGraph& g) {
  ...
  for (auto& ed : g.getEdges()) {
    ...
    // cout << (c.first).toString() << " ---> " << (c.second).toString() <<
    // endl;

    // Either the first edge is not a register or it is not a receiver
    Wireable*
      fstParent = g.getNode(source)
                    .getWire();  // toSelect(*(c.first.getWire())).getParent();
    ...
    Wireable*
      sndParent = g.getNode(target)
                    .getWire();  // toSelect(*(c.second.getWire())).getParent();
...
TEST_CASE("Sequential logic") {
  ...
  SECTION("Register with fanout 2") {
    ...
    Type* regType = c->Record({{"clk", c->Named("coreir.clkIn")},
    ...
    if (!saveToFile(
          c->getGlobal(), "register_fan_out_2_pre_gen.json", regComb)) {
    ...
    SECTION("Compile and run") {
      ...
      int s = compileCodeAndRun(
        topoOrder, g, regComb, "./gencode/", outFile, testFile);
  ...
  SECTION("Combinational logic before register update, sequential path") {
    ...
    if (!saveToFile(
          c->getGlobal(), "comb_then_register_seq_only.json", regComb)) {
      cout << "Could not save to json!!" << endl;
      c->die();
    }
  ...
  SECTION(
    "Combinational logic before register update, combinational and sequential "
    "path") {
    uint width = 32;
  ...
  SECTION("Memory primitive") {
    ...
    SECTION("Compile and run") {
      int s = compileCodeAndRun(
        topoOrder, g, memory, "./gencode/", "memory", "test_memory.cpp");
  ...
  SECTION("Registers with enable, but the enable is connected to a constant") {
    ...
    Type* RegType = c->Record({{"en", c->BitIn()},
    ...
    def->addInstance(
      "r0",
      "mantle.reg",
      {{"width", Const::make(c, n)}, {"has_en", Const::make(c, true)}});
    def->addInstance(
      "r1",
      "mantle.reg",
      {{"width", Const::make(c, n)}, {"has_en", Const::make(c, true)}});
    ...
    def->addInstance(
      "en_const", "corebit.const", {{"value", Const::make(c, true)}});
    ...
    SECTION("Compile and run") {
      ...
      int s = compileCodeAndRun(
        topoOrder, g, rg, "./gencode/", outFile, "test_reg_const_enable.cpp");
  ...
  SECTION("Non standard width register") {
    ...
    SECTION("Compile and run") {
      ...
      int s = compileCodeAndRun(
        topoOrder, g, rg, "./gencode/", outFile, "test_reg5.cpp");
  ...
  SECTION("Counter") {
    ...
    // Now lets create a module declaration. Declarations are specified
    // separately from the definition
    Module* counter = c->getGlobal()->newModuleDecl(
      "counter",
      CounterType);  // use getGlobalFunction
    ...
    def->addInstance(
      "ai",
      "coreir.add",
      wArg);  // using <namespace>.<module> notation
    def->addInstance(
      "ci",
      "coreir.const",
      wArg,
      {{"value", Const::make(c, BitVector(16, 1))}});

    // Reg has default arguments. en/clr/rst are False by default. Init is also
    // 0 by default
    def->addInstance(
      "ri",
      "mantle.reg",
    ...
    SECTION("Compile and run") {
      ...
      int s = compileCodeAndRun(
        topoOrder, g, counter, "./gencode/", outFile, "test_counter.cpp");
  ...
  SECTION("Register chain") {
    ...
    SECTION("Compile and run") {
      int s = compileCode(
        topoOrder, g, regChain, "./gencode/", "register_chain");
      REQUIRE(s == 0);
    }
  }
  ...
  SECTION("Register without enable") {
    ...
    def->addInstance(
      "r0",
      "mantle.reg",
      {{"width", Const::make(c, n)}, {"has_en", Const::make(c, false)}});
    ...
    SECTION("Compile and run") {
      int s = compileCode(
        topoOrder, g, regChain, "./gencode/", "register_no_enable");
      REQUIRE(s == 0);
    }
  }
  ...
  SECTION("LineBufferMem") {
    ...
    def->addInstance("lb_wen", "corebit.const", {{"value", Const::make(c,
  true)}}); def->addInstance("m0", "memory.rowbuffer",
                     {{"width", Const::make(c, width)},
  ...
  */
  ...
  //     cout << preG.getNode(vd).getWire()->toString() << " has " <<
  //     preG.outEdges(vd).size() << " outputs " << endl;
  //   }
  ...
  //       cout << wd.getWire()->toString() << " : " <<
  //       wd.getWire()->getType()->toString() << endl;
  ...
  //       cout << wd.getWire()->toString() << " : " <<
  //       wd.getWire()->getType()->toString() << " has " <<
  //       g.outEdges(vd).size() << " outputs" << endl;
