...
void setThreadNumbers(NGraph& gr) {
  for (auto& v : gr.getVerts()) {
    ...
    // cout << "Thread number before setting = " << gr.getNode(v).getThreadNo()
    // << endl;
    gr.addVertLabel(v, w);

    // cout << "Thread number after setting  = " << gr.getNode(v).getThreadNo()
    // << endl;
  }
}
...
TEST_CASE("Large circuits for testing") {
  ...
  SECTION("Many logical operations in parallel") {
    ...
    // Define Add4 Module
    RecordParams opParams = {{"clk", c->Named("coreir.clkIn")}};
    ...
    for (uint i = 0; i < numOutputs; i++) {
      ...
      if ((i % 2) == 0) {
        op = def->addInstance(
          "and_" + to_string(i), and2, {{"width", Const::make(c, n)}});
      }
      else {
        op = def->addInstance(
          "or_" + to_string(i), or2, {{"width", Const::make(c, n)}});
      }

      auto reg = def->addInstance(
        "reg_" + to_string(i), "coreir.reg", {{"width", Const::make(c, n)}});
    ...
    for (uint i = 0; i < numOutputs; i++) {
      ...
      if ((i % 1000) == 0) { cout << "Selects " << i << "!!!" << endl; }
    }
    ...
    for (uint i = 0; i < numOutputs; i++) {
      ...
      if ((i % 1000) == 0) { cout << "Wired up inputs " << i << endl; }
    }
