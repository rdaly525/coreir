...
#define CATCH_REC_LIST0(f, x, peek, ...)                                       \
  , f(x)                                                                       \
      CATCH_DEFER(CATCH_REC_NEXT(peek, CATCH_REC_LIST1))(f, peek, __VA_ARGS__)
#define CATCH_REC_LIST1(f, x, peek, ...)                                       \
  , f(x)                                                                       \
      CATCH_DEFER(CATCH_REC_NEXT(peek, CATCH_REC_LIST0))(f, peek, __VA_ARGS__)
...
#define INTERNAL_CATCH_REMOVE_PARENS_11_ARG(                                   \
  ...
  INTERNAL_CATCH_REMOVE_PARENS(_0), INTERNAL_CATCH_REMOVE_PARENS_10_ARG(       \
...
#define INTERNAL_CATCH_TYPE_GEN                                                \
  ...
  constexpr auto rewrap(List<Elems...>, Elements...) noexcept->decltype(       \
    append(                                                                    \
      TypeList<Container<Elems...>>{}, rewrap<Container>(Elements{}...))) {    \
    return {};                                                                 \
  }                                                                            \
                                                                               \
  template <                                                                   \
...
#define INTERNAL_CATCH_NTTP_REGISTER0(TestFunc, signature)                     \
  template <typename Type> void reg_test(                                      \
    TypeList<Type>, Catch::NameAndTags nameAndTags) {                          \
    Catch::AutoReg(                                                            \
...
#define INTERNAL_CATCH_NTTP_REGISTER(TestFunc, signature, ...)                 \
  template <INTERNAL_CATCH_REMOVE_PARENS(signature)> void reg_test(            \
    Nttp<__VA_ARGS__>, Catch::NameAndTags nameAndTags) {                       \
    Catch::AutoReg(                                                            \
...
#define INTERNAL_CATCH_DEFINE_SIG_TEST_METHOD1(TestName, signature)            \
  template <typename TestType> void INTERNAL_CATCH_MAKE_NAMESPACE(             \
...
struct is_callable_tester {
  template <typename Fun, typename... Args> true_given<
...
template <typename C> auto makeTestInvoker(void (C::*testAsMethod)()) noexcept
  -> ITestInvoker* {
...
class ReusableStringStream {
 ...
 public:
  ...
  template <typename T> auto operator<<(T const& value)
    -> ReusableStringStream& {
    *m_oss << value;
    return *this;
  }
...
struct IMutableEnumValuesRegistry {
  ...
  template <typename E> Detail::EnumInfo const& registerEnum(
...
template <typename T, typename = void> struct StringMaker {
  template <typename Fake = T> static typename std::enable_if<
...
template <typename T> struct StringMaker<T*> {
  template <typename U> static std::string convert(U* p) {
    if (p) { return ::Catch::Detail::rawMemoryToString(p); }
    else {
      return "nullptr";
    }
  }
...
template <typename T> struct StringMaker<std::optional<T>> {
  static std::string convert(const std::optional<T>& optional) {
    ReusableStringStream rss;
    if (optional.has_value()) { rss << ::Catch::Detail::stringify(*optional); }
    else {
      rss << "{ }";
    }
    return rss.str();
...
template <typename... Elements> struct StringMaker<std::variant<Elements...>> {
  static std::string convert(const std::variant<Elements...>& variant) {
    if (variant.valueless_by_exception()) { return "{valueless variant}"; }
    else {
      return std::visit(
...
template <typename R> struct StringMaker<
  R,
...
template <typename Duration> struct StringMaker<
...
#define INTERNAL_CATCH_REGISTER_ENUM(enumName, ...)                            \
  ...
  template <> struct StringMaker<enumName> {                                   \
    static std::string convert(enumName value) {                               \
      static const auto& enumInfo = ::Catch::getMutableRegistryHub()           \
...
template <typename LhsT, typename RhsT> class BinaryExpr
...
template <typename LhsT> class ExprLhs {
 ...
 public:
  ...
  template <typename RhsT> auto operator==(RhsT const& rhs)
    -> BinaryExpr<LhsT, RhsT const&> const {
  ...
  template <typename RhsT> auto operator&&(RhsT const&)
    -> BinaryExpr<LhsT, RhsT const&> const {
    static_assert(
...
#define INTERNAL_CATCH_SECTION(...)                                            \
  ...
  if (                                                                         \
    Catch::Section const&                                                      \
      INTERNAL_CATCH_UNIQUE_NAME(catch_internal_Section) = Catch::SectionInfo( \
...
using ExceptionTranslators = std::vector<
  std::unique_ptr<IExceptionTranslator const>>;
...
template <typename T> Generic::PredicateMatcher<T> Predicate(
...
namespace Vector {
template <typename T> struct ContainsElementMatcher
...
template <typename T> struct UnorderedEqualsMatcher
  : MatcherBase<std::vector<T>> {
...
template <typename T> Vector::UnorderedEqualsMatcher<T> UnorderedEquals(
...
template <typename ArgT, typename MatcherT> class MatchExpr
  : public ITransientExpression {
  ArgT const& m_arg;
  MatcherT m_matcher;
...
template <typename ArgT, typename MatcherT> auto makeMatchExpr(
  ArgT const& arg, MatcherT const& matcher, StringRef const& matcherString)
...
template <typename T, typename... Gs> auto makeGenerators(
  GeneratorWrapper<T>&& generator, Gs... moreGenerators) -> Generators<T> {
  return Generators<T>(
    std::move(generator), std::forward<Gs>(moreGenerators)...);
}
template <typename T> auto makeGenerators(GeneratorWrapper<T>&& generator)
  -> Generators<T> {
...
template <typename T, typename Predicate> class FilterGenerator
  : public IGenerator<T> {
  GeneratorWrapper<T> m_generator;
...
template <typename T, typename U, typename Func> class MapGenerator
  : public IGenerator<T> {
...
template <typename T> class ChunkGenerator final
...
template <typename T> GeneratorWrapper<std::vector<T>> chunk(
...
template <typename Float> class RandomFloatingGenerator final
...
template <typename Integer> class RandomIntegerGenerator final
...
template <typename DerivedT> struct CumulativeReporterBase
  : IStreamingReporter {
...
template <typename Fun, typename... Args>
CompleteType_t<ResultOf_t<Fun(Args...)>> complete_invoke(
  Fun&& fun, Args&&... args) {
...
template <typename Clock> struct ChronometerModel final
...
template <typename Duration> struct EnvironmentEstimate {
  ...
  template <typename Duration2> operator EnvironmentEstimate<Duration2>()
    const {
...
template <typename T, typename U> struct is_related
...
struct BenchmarkFunction {
 ...
 public:
  ...
  template <
    typename Fun,
    typename std::enable_if<!is_related<Fun, BenchmarkFunction>::value, int>::
      type = 0>
...
template <typename Clock, typename Sig> using TimingOf =
...
template <typename Duration> struct ExecutionPlan {
  ...
  template <typename Clock> std::vector<FloatDuration<Clock>> run(
...
template <typename URng, typename Iterator, typename Estimator> sample resample(
  URng& rng,
  int resamples,
  Iterator first,
  Iterator last,
...
template <typename Iterator, typename Estimator> Estimate<double> bootstrap(
  ...
  std::tie(sum_squares, sum_cubes) = std::accumulate(
    ...
    std::make_pair(0., 0.),
    [jack_mean](
      std::pair<double, double> sqcb, double x) -> std::pair<double, double> {
      auto d = jack_mean - x;
      auto d2 = d * d;
      auto d3 = d2 * d;
...
struct Benchmark {
  ...
  template <typename Clock> ExecutionPlan<FloatDuration<Clock>> prepare(
    const IConfig& cfg, Environment<FloatDuration<Clock>> env) const {
 ...
 public:
  class iterator {
    ...
    void calcLength() {
      ...
      if (m_end < m_pos + width) { m_len = m_end - m_pos; }
      else {
        size_t len = width;
...
// lambdas)
template <typename L> struct UnaryLambdaTraits
...
inline auto convertInto(std::string const& source, bool& target)
  ...
  if (
    srcLC == "y" || srcLC == "1" || srcLC == "true" || srcLC == "yes" ||
    srcLC == "on")
    target = true;
  else if (
...
template <typename T> inline auto convertInto(
...
template <typename ArgType, typename L> inline auto invokeLambda(
  L const& lambda, std::string const& arg) -> ParserResult {
  ArgType temp{};
  auto result = convertInto(arg, temp);
  return !result
           ? result
...
template <typename DerivedT> class ParserRefImpl
...
class ExeName : public ComposableParserImpl<ExeName> {
 ...
 public:
  ...
  template <typename LambdaT> explicit ExeName(LambdaT const& lambda)
    : ExeName() {
...
class Opt : public ParserRefImpl<Opt> {
 ...
 public:
  ...
  template <typename T> Opt(T& ref, std::string const& hint)
    : ParserRefImpl(ref, hint) {}
  ...
  auto parse(std::string const& exeName, TokenStream const& tokens) const
    ...
    while (result.value().remainingTokens()) {
      ...
      for (size_t i = 0; i < totalParsers; ++i) {
        ...
        if (
          parseInfo.parser->cardinality() == 0 ||
...
clara::Parser makeCommandLineParser(ConfigData& config) {
  ...
  auto
    cli = ExeName(config.processName) | Help(config.showHelp) |
...
AssertionStats::AssertionStats(
  ...
  : assertionResult(_assertionResult),
    ...
    totals(_totals) {
  assertionResult.m_resultData.lazyExpression
    .m_transientExpression = _assertionResult.m_resultData.lazyExpression
...
template <typename FP> FP step(FP start, FP direction, int steps) {
  for (int i = 0; i < steps; ++i) { start = std::nextafter(start, direction); }
  return start;
}
...
std::string WithinUlpsMatcher::describe() const {
  ...
  if (m_type == FloatingPointKind::Float) { ret << 'f'; }
...
std::string Catch::Matchers::Generic::Detail::finalizeDescription(
  ...
  if (desc.empty()) { return "matches undescribed predicate"; }
  else {
...
struct GeneratorTracker : TestCaseTracking::TrackerBase, IGeneratorTracker {
  ...
  static GeneratorTracker& acquire(
    ...
    if (
      TestCaseTracking::ITrackerPtr childTracker = currentTracker.findChild(
        nameAndLocation)) {
      assert(childTracker);
...
void RunContext::handleExpr(
  ...
  if (result) {
    if (!m_includeSuccessfulResults) { assertionPassed(); }
    else {
...
#if !defined(CATCH_CONFIG_DISABLE_EXCEPTIONS)
  const auto&
    exceptions = getRegistryHub().getStartupExceptionRegistry().getExceptions();
  if (!exceptions.empty()) {
    config();
...
void SectionTracker::tryOpen() {
  if (
    !isComplete() && (m_filters.empty() || m_filters[0].empty() ||
...
std::string StringMaker<char const*>::convert(char const* str) {
  if (str) { return ::Catch::Detail::stringify(std::string{str}); }
  else {
    return {"{null string}"};
  }
}
...
std::string StringMaker<wchar_t const*>::convert(wchar_t const* str) {
  if (str) { return ::Catch::Detail::stringify(std::wstring{str}); }
  else {
    return {"{null string}"};
  }
}
...
void printTotals(std::ostream& out, const Totals& totals) {
  if (totals.testCases.total() == 0) { out << "No tests ran."; }
  else if (totals.testCases.failed == totals.testCases.total()) {
    Colour colour(Colour::ResultError);
    const std::string
      qualify_assertions_failed = totals.assertions.failed ==
...
std::size_t makeRatio(std::size_t number, std::size_t total) {
  std::size_t ratio = total > 0 ? CATCH_CONFIG_CONSOLE_WIDTH * number / total
                                : 0;
...
bool JunitReporter::assertionEnded(AssertionStats const& assertionStats) {
  if (
    assertionStats.assertionResult.getResultType() ==
...
void JunitReporter::writeSection(
  ...
  if (
    !sectionNode.assertions.empty() || !sectionNode.stdOut.empty() ||
