...
#define CATCH_REC_LIST0_UD(f, userdata, x, peek, ...)                          \
  ,                                                                            \
    f(userdata, x) CATCH_DEFER(CATCH_REC_NEXT(                                 \
      peek,                                                                    \
      CATCH_REC_LIST1_UD))(f, userdata, peek, __VA_ARGS__)
#define CATCH_REC_LIST1_UD(f, userdata, x, peek, ...)                          \
  ,                                                                            \
    f(userdata, x) CATCH_DEFER(CATCH_REC_NEXT(                                 \
      peek,                                                                    \
      CATCH_REC_LIST0_UD))(f, userdata, peek, __VA_ARGS__)
...
#define INTERNAL_CATCH_TYPE_GEN                                                \
  ...
  constexpr auto append(L1<E1...>, TypeList<mpl_::na>, Rest...) noexcept       \
    ->L1<E1...> {                                                              \
    return {};                                                                 \
  }                                                                            \
                                                                               \
...
#define INTERNAL_CATCH_NTTP_1(signature, ...)                                  \
  ...
  template <                                                                   \
    ...
    typename... Elements>                                                      \
  constexpr auto                                                               \
  rewrap(List<__VA_ARGS__>, Elements... elems) noexcept->decltype(             \
...
void printTotals(std::ostream& out, const Totals& totals) {
  if (totals.testCases.total() == 0) { out << "No tests ran."; }
    ...
    const std::string qualify_assertions_failed = totals.assertions.failed ==
