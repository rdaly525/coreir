...
std::string primitiveRandomInputString(
  CoreIR::Type& t, const std::string& var) {
...
std::string randomInputString(CoreIR::Type& tp, const std::string& var) {
  if (isPrimitiveType(tp)) { return primitiveRandomInputString(tp, var); }
...
std::string randomSimInputString(Module* mod) {
  ...
  string res = "";
  for (auto& arg : args) { res += randomInputString(*(arg.first), arg.second); }
...
std::vector<std::pair<CoreIR::Type*, std::string>> simOutputVarDecls(
  Module& mod) {
  Type* tp = mod.getType();
...
std::string callSimulate(Module& mod) {
  ...
  res += ln(
    "std::cout << \"Time: \" << (std::clock() - start) / "
    "(double)(CLOCKS_PER_SEC / 1000) << \" ms\" << std::endl");
...
int generateHarnessAndRun(
  ...
  string runCmd = "clang++ -lpthread -std=c++11 " + codeFilePath + " " +
                  harnessFile;
...
int buildVerilator(Module* m, Namespace* g) {
  ...
  string runCmd = "../../bin/coreir -i " + jsonFile + " " + " -o " +
                  verilogFile;
  int s = system(runCmd.c_str());
