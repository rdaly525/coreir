...
namespace CoreIR {
TEST_CASE("basic module passing test") {
  ...
  SECTION("basic section") {
    ...
    g->newTypeGen(
      ...
      {{"N", c->Int()}, {"op", ModuleType::make(c)}},  // generater parameters
      [](Context* c, Values args) {  // Function to compute type
        ...
        return c->Record({{"in", c->BitIn()->Arr(width)->Arr(2)->Arr(N)},
                          {"out", c->Bit()->Arr(width)->Arr(N)}});
      });

    Generator* map = g->newGeneratorDecl(
      "map",
      g->getTypeGen("map_type"),
      {{"N", c->Int()}, {"op", ModuleType::make(c)}});
    map->setGeneratorDefFromFun([](Context* c, Values args, ModuleDef* def) {
      ...
      for (uint i = 0; i < N; ++i) {
        ...
        def->connect(
          opN->sel("in0"), def->getInterface()->sel("in")->sel(i)->sel(0));
        def->connect(
          opN->sel("in1"), def->getInterface()->sel("in")->sel(i)->sel(1));
    ...
    Module* mul = c->getGenerator("coreir.mul")
                    ->getModule({{"width", Const::make(c, 13)}});

    Instance* map_mul = def->addInstance(
      "map_mul",
      "global.map",
      {{"N", Const::make(c, 5)}, {"op", Const::make(c, mul)}});
    test->setDef(def);
    test->print();
