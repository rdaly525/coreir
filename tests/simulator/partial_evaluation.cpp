...
class PartialEvaluator {
 ...
 public:
  ...
  void setValue(CoreIR::Select* const sel, const BitVec& bv) {
  ...
  SimValue* getValue(CoreIR::Select* sel) {
    if (arrayAccess(sel)) {
      ...
      return makeSimBitVector(
        BitVec(1, (val->getBits()).get(index).binary_value()));
    }
    ...
    if (it == std::end(current.valMap)) { return nullptr; }
  ...
  void setValue(CoreIR::Select* const sel, SimValue* val) {
...
TEST_CASE("Partial evaluation") {
  ...
  SECTION("Partially evaluate a mux whose select is a register") {
    ...
    uint width = 8;
    Type* combLoop = c->Record({{"clk", c->Named("coreir.clkIn")},
                                {"regIn", c->BitIn()->Arr(1)},
    ...
    for (auto dp : sigDrivers) {
      cout << "\t" << dp.first->toString() << " driven by "
           << dp.second->toString() << endl;
    }
    ...
    for (auto dp : sigReceivers) {
      ...
      for (auto val : dp.second) { cout << val->toString() << ", " << endl; }
    }
  ...
  SECTION("Partially evaluating a register") {
    ...
    Type* regOut = c->Record({{"clk", c->Named("coreir.clkIn")},
