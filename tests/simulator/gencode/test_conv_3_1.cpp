...
int main() {
  ...
  for (int i = 0; i < nRuns; i++) {
    ...
    if ((state.self_clk_last == 0) && ((i + 1) % 2 == 1)) {
      cout << "out " << i << " = " << state.self_out << endl;
    }

    if ((state.self_clk_last == 1) && ((i + 1) % 2 == 0)) {
      state.self_in_0 = (state.self_in_0 + 1) % 25;
    }

    if (i == 40) { assert(state.self_out == 205); }
  }
  ...
  double time_ms = (end - start) / (double)(CLOCKS_PER_SEC / 1000);

  cout << "Time to compute " << nRuns << " half cycles = " << time_ms << " ms"
       << endl;
