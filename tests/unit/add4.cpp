...
int main() {
  ...
  ModuleDef* def = add4_n->newModuleDef();
    ...
    Wireable* add_00 = def->addInstance("add00",add2,c->newGenArgs({{"w",c->GInt(n)}}),nullptr);
    Wireable* add_01 = def->addInstance("add01",add2,c->newGenArgs({{"w",c->GInt(n)}}),nullptr);
    Wireable* add_1 = def->addInstance("add1",add2,c->newGenArgs({{"w",c->GInt(n)}}),nullptr);
