...
int main() {
  ...
  Generator* addN = g->newGeneratorDecl(
    "addN", g->getTypeGen("addN_type"), {{"width", c->Int()}, {"N", c->Int()}});
  ...
  addN->setGeneratorDefFromFun([](Context* c, Values args, ModuleDef* def) {
    ...
    if (N == 2) {
      ...
      for (uint i = 0; i < N / 2; ++i) {
        def->connect(
          {"self", "in", to_string(i)}, {"addN_0", "in", to_string(i)});
        def->connect(
          {"self", "in", to_string(i + N / 2)}, {"addN_1", "in", to_string(i)});
      }
  ...
  def->addInstance(
    "add2_join", coreir->getGenerator("add"), {{"width", Const::make(c, 13)}});
