...
int main() {
  ...
  string constModule = Aetherling_addCoreIRConstantModule(
    c, mulBy2Def, width, Const::make(c, width, 4));
  mulBy2Def->addInstance(
    "mul", "coreir.mul", {{"width", Const::make(c, width)}});
  ...
  Values mapNParams({{"width", Const::make(c, width)},
  ...
  Module* add = c->getGenerator("coreir.add")
                  ->getModule({{"width", Const::make(c, width)}});
