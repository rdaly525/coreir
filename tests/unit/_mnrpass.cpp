...
int main() {
  ...
  def->addInstance(
    "c0", sl->getGenerator("const"), wargs, {{"value", Const::make(c, 5)}});
  def->addInstance(
    "c1", sl->getGenerator("const"), wargs, {{"value", Const::make(c, 5)}});
  def->addInstance(
    "c2", sl->getGenerator("const"), wargs, {{"value", Const::make(c, 2)}});
  ...
  def = ma->newModuleDef();
  def->addInstance(
    "c0", sl->getGenerator("const"), wargs, {{"value", Const::make(c, 0)}});
  def->addInstance(
    "c1", sl->getGenerator("const"), wargs, {{"value", Const::make(c, 1)}});
  def->addInstance(
    "c2", sl->getGenerator("const"), wargs, {{"value", Const::make(c, 2)}});
  ...
  opts.genargs = wargs;
  MatchAndReplace* pSub2Add = new MatchAndReplace(
    "sub2add", patternSub, sl->getGenerator("add"), opts);
  c->addPass(pSub2Add);
  ...
  pdef->addInstance(
    "c0", sl->getGenerator("const"), wargs, {{"value", Const::make(c, 31)}});
  pdef->connect("self.in", "a0.in1");
  pdef->connect("c0.out", "a0.in0");
  pdef->connect("self.out", "a0.out");
  patternC->setDef(pdef);

  MatchAndReplace* pC = new MatchAndReplace(
    "pC", patternC, sl->getGenerator("neg"), opts);
  //(void) pC;
  c->addPass(pC);
