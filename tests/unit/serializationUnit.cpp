...
int main() {
  ...
  // Type of module
  Type* oneInManyOutGenType = c->Record({{"in", allInputsType},
  ...
  Values serializerParams{{"rate", Const::make(c, parallelInputs)},
                          {"width", Const::make(c, inputWidth)}};

  testDef->addInstance("serializer", "commonlib.serializer", serializerParams);
  testDef->addInstance(
    "deserializer", "commonlib.deserializer", serializerParams);
