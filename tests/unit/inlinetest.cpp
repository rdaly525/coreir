...
int main() {
  ...
  g->newTypeGen(
    ...
    [](Context* c, Values args) {  // Function to compute type
      ...
      return c->Record({{"in", c->BitIn()->Arr(width)->Arr(4)},
                        {"out", c->Bit()->Arr(width)}});
    });

  Generator* add4 = g->newGeneratorDecl(
    "add4", g->getTypeGen("add4_type"), {{"width", c->Int()}});
  ...
  Type* t = g->getTypeGen("add4_type")
              ->getType({{"width", Const::make(c, 13)}});

  Module* add = g->newModuleDecl("Add", t);
  ModuleDef* def = add->newModuleDef();
  Instance* inst = def->addInstance(
    "i0", add4, {{"width", Const::make(c, 13)}});
  for (uint i = 0; i < 4; ++i) {
    def->connect(
      inst->sel("in")->sel(i), def->getInterface()->sel("in")->sel(i));
  }
