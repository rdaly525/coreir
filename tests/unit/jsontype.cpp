...
int main() {
  ...
  g->newGeneratorDecl(
    "addN", g->getTypeGen("json_type"), {{"width", JsonType::make(c)}});
  ...
  if (!saveToFile(g, "_jsontype.json", top)) {
  ...
  c = newContext();
  cout << "loading" << endl;
  if (!loadFromFile(c, "_jsontype.json")) {
    cout << "Could not Load from json!!" << endl;
    c->die();
  }
  ASSERT(
    jdata == c->getModule("global.top")
               ->getDef()
               ->getInstances()
               .at("inst")
               ->getModuleRef()
