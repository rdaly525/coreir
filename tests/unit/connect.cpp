...
int main() {
  ...
  Module* const16 = coreir->getGenerator("const")->getModule(
    {{"width", Const::make(c, 16)}});

  // Define Module Type
  Type* mType = c->Record({{"out", c->Array(16, c->Bit())}});

  Module* mod = g->newModuleDecl("mod", mType);
  ModuleDef* def = mod->newModuleDef();
  Wireable* self = def->sel("self");
  Wireable* i0 = def->addInstance(
    "i0", const16, {{"value", Const::make(c, BitVector(16, 23))}});
  ...
  cout << "metadata: " << def->getMetaData(i0->sel("out"), self->sel("out"))
       << endl;
