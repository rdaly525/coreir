...
void getPortList(
  ...
  if (isBitOrArrOfBits(t)) {
    ...
    // Get the flattened port list generated by the first child
    vector<std::pair<SelectPath, Type*>> elem_ports;
    SelectPath next = cur;
    int curr_idx = next.size();
    next.push_back("0");
    // uports should be unused
    getPortList(at->getElemType(), next, elem_ports, uports);

    // Copy the flattened port list for every child
    for (uint i = 0; i < at->getLen(); ++i) {
      for (auto port : elem_ports) {
        SelectPath child_sp = port.first;
        child_sp[curr_idx] = to_string(i);
        ports.push_back({child_sp, port.second});
      }
    }
  }
