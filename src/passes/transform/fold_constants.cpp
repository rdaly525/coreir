...
bool foldConstants(CoreIR::Module* const mod) {
  ...
  while (toConsider.size() > 0) {
    ...
    if (!contains_key(inst->toString(), insts)) { continue; }
    ...
    if (getQualifiedOpName(*(inst)) == "coreir.mux") {
      ...
      if (
        ...
        if (!val.get(offset).is_binary()) { continue; }
        ...
        if (!val.get(0).is_binary()) { continue; }
      ...
      if (isa<Instance>(p)) {
        ...
        if (getQualifiedOpName(*rstSrcInst) == "coreir.wrap") {
          ...
          // If the reset port is set to a constant then we may be able to
          // constant fold
...
bool Passes::FoldConstants::runOnModule(Module* m) { return foldConstants(m); }
