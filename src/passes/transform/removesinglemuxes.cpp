...
namespace CoreIR {
  ...
  namespace Passes {
    ...
    bool RemoveSingleMuxes::runOnModule(Module* m) {
      ...
      for (auto instR : def->getInstances()) {
        ...
        if (getQualifiedOpName(*inst) == "commonlib.muxn") {
          ...
          if (n == 1) {
            ...
            if (out.second->getTopParent() == inst) {
              down = out.first;
            }

            // should only have two connections
            for (auto conn : getSourceConnections(inst)) {
              if (((Select *)(conn.first))->getSelStr() != "sel" && ((Select *)(conn.second))->getSelStr() != "sel") {
                // upstream connection
                auto up = conn.first;
                if (conn.first->getTopParent() == inst) {
                  up = conn.second;
                }
      ...
      for (auto mux : singleMuxes) {
        def->removeInstance(mux);
      }
      for (uint i = 0; i < selects.size(); i++) {
