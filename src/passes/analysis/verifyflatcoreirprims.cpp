...
bool Passes::VerifyFlatCoreirPrims::runOnInstanceGraphNode(
  ...
  for (auto inst : node.getInstanceList()) {
    ...
    if (m->isGenerated()) { ns = m->getGenerator()->getNamespace(); }
    else {
      ns = m->getNamespace();
    }
    ASSERT(
      ns == coreir || ns == corebit || ns == mantle,
      "Expected flattened design and {" + inst->getInstname() +
        ", namespace= " + ns->getName() + "} is not a recognized primitive.");
  }
  return false;
}
