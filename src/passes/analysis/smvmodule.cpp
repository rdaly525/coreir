...
string SMVModule::toInstanceString(Instance* inst, string path) {
  ...
  if (gen) {
    ...
    mname =
      modname; // gen->getNamespace()->getName() + "_" + gen->getName(args);
  }
  else {
    mname = modname;
  }

  // Even if not generator, there are some arguments in GenArgs and we want
  // those too
...
#define var_assign(var, name)                                                  \
  if (portstrs.find(name) != portstrs.end()) var = portstrs.find(name)->second
  ...
  case const_op:
    int value;
    value = stoi(args["value"]->toString());
  ...
  case slice_op:
    int lo;
    lo = stoi(args["lo"]->toString());
    int hi;
    hi = stoi(args["hi"]->toString());
