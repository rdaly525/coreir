...
std::string CoreIRVModule::inline_instance(
  ...
  if (
    ...
    if (vmods->_inline && right_parent_verilog_module->inlineable) {
      ...
      right_conn_str =
        std::regex_replace(right_conn_str, std::regex("assign out = "), "");
      ...
      for (auto record_pair :
        ...
        if (record_pair.second->getDir() == Type::DK_In) {
          std::string replace =
            get_replace_str(record_pair.first, right_parent, def, worklist);
...
void CoreIRVModule::addConnectionsInlined(ModuleDef* def) {
  ...
  while (!worklist.empty()) {
    ...
    worklist.pop();
    Wireable* left =
      conn.first->getType()->getDir() == Type::DK_In ? conn.first : conn.second;
    ...
    if (!(left->getSelectPath()[0] == "self" &&
      ...
      if (Instance* right_parent = dyn_cast<Instance>(right->getTopParent())) {
        right_conn_str =
          CoreIRVModule::inline_instance(def, worklist, right_parent);
      }
      else {
        ASSERT(
          right->getSelectPath()[0] == "self",
...
string VModule::toString() const {
  ...
  for (auto p : params) {
    ...
    if (p != "type") {
      string defaultVal =
        paramDefaults.count(p) > 0 ? paramDefaults.at(p) : "1";
