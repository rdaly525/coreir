...
string unary_op(string op, string in) { return "(" + op + " " + in + ")"; }
...
string get_init(string init) { return "INIT" + NL + init + ";"; }

string get_trans(string trans) { return "TRANS" + NL + trans + ";"; }
...
string SMVgetCurr(string context, string var) {
  return "\"" + context + var + "\"";
}
string SMVgetNext(string context, string var) {
  return "next(" + SMVgetCurr(context, var) + ")";
}
...
string
SMVConcat(string context, SmvBVVar in1_p, SmvBVVar in2_p, SmvBVVar out_p) {
  return SMVBop(context, "Concat", "::", in1_p, in2_p, out_p);
}

string
SMVSlice(string context, SmvBVVar in_p, SmvBVVar out_p, int low_p, int high_p) {
  // INVAR: (in[high:low] = out)
  string in = in_p.getPortName();
  string out = out_p.getPortName();
  string low = to_string(low_p);
  string high = to_string(high_p);
  string comment = "-- SMVSlice (in, out, low, high) = (" + in + ", " + out +
                   ", " + low + ", " + high + ")";
  string op = "[" + high + ":" + low + "]";
...
string SMVNot(string context, SmvBVVar in_p, SmvBVVar out_p) {
  ...
  string op = "!";
  string curr =
    unary_op_eq(op, SMVgetCurr(context, in), SMVgetCurr(context, out));
  return comment + NL + get_invar(curr);
}
...
string SMVReg(string context, SmvBVVar in_p, SmvBVVar clk_p, SmvBVVar out_p) {
  ...
  string trans =
    "(((!{clk} & next({clk})) = 0ud1_1) -> (next({out}) = {in})) & ((!(!{clk} "
    "& next({clk})) = 0ud1_1) -> (next({out}) = {out}))";
...
string SMVRegPE(
  ...
  string comment = "-- SMVRegPE (in, clk, out, en) = (" + in + ", " + clk +
                   ", " + out + ", " + en + ")";
...
string SMVMux(
  ...
  string out = out_p.getPortName();
  string comment = "-- SMVMux (in0, in1, sel, out) = (" + in0 + ", " + in1 +
                   ", " + sel + ", " + out + ")";
