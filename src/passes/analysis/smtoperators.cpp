...
string SMTBop(
  ...
  string comment = ";; SMT" + opname + " (in1, in2, out) = (" + in1 + ", " +
                   in2 + ", " + out + ")";
  string curr = binary_op_eqass(
    op,
    SMTgetCurr(context, in1),
    SMTgetCurr(context, in2),
    SMTgetCurr(context, out));
  string next = binary_op_eqass(
    op,
    SMTgetNext(context, in1),
...
string SMTEq(string context, SmtBVVar in1_p, SmtBVVar in2_p, SmtBVVar out_p) {
  ...
  string out = out_p.getPortName();
  string comment =
    ";; SMT Eq(in1, in2, out) = (" + in1 + ", " + in2 + ", " + out + ")";
...
string
SMTConcat(string context, SmtBVVar in1_p, SmtBVVar in2_p, SmtBVVar out_p) {
...
string SMTUop(
  ...
  string curr =
    unary_op_eqass(op, SMTgetCurr(context, in), SMTgetCurr(context, out));
  string next =
    unary_op_eqass(op, SMTgetNext(context, in), SMTgetNext(context, out));
  return comment + NL + curr + NL + next;
}

string
SMTSlice(string context, SmtBVVar in_p, SmtBVVar out_p, int low_p, int high_p) {
...
string SMTClock(string context, SmtBVVar clk_p) {
  ...
  string trans = assert_op(
    "(= " + SMTgetCurr(context, clk) + " (bvnot " + SMTgetNext(context, clk) +
    "))");
  return comment + NL + init + NL + trans;
}
