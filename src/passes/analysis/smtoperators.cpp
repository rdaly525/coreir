...
string assert_op(string expr) { return "(assert " + expr + ")"; }
...
string SMTgetInit(string context, string var) {
  return context + var + INIT_PF;
}
string SMTgetCurr(string context, string var) {
  return context + var + CURR_PF;
}
string SMTgetNext(string context, string var) {
  return context + var + NEXT_PF;
}
...
string SMTAssign(SmtBVVar vleft, SmtBVVar vright) {
  ...
  string curr = assert_op(
      binary_op("=", vleft_c.getExtractName(), vright_c.getExtractName()));
  string next = assert_op(
      binary_op("=", vleft_n.getExtractName(), vright_n.getExtractName()));
  return curr + NL + next;
}

string SMTBop(string context, string opname, string op, SmtBVVar in1_p,
              SmtBVVar in2_p, SmtBVVar out_p) {
  // INIT: TRUE
  // TRANS: ((in1 <op> in2) = out) & ((in1' & in2') = out')
  string in1 = in1_p.getPortName();
  string in2 = in2_p.getPortName();
  string out = out_p.getPortName();
  string comment = ";; SMT" + opname + " (in1, in2, out) = (" + in1 + ", " +
                   in2 + ", " + out + ")";
  string curr = binary_op_eqass(op, SMTgetCurr(context, in1),
                                SMTgetCurr(context, in2),
                                SMTgetCurr(context, out));
  string next = binary_op_eqass(op, SMTgetNext(context, in1),
                                SMTgetNext(context, in2),
...
string SMTEq(string context, SmtBVVar in1_p, SmtBVVar in2_p, SmtBVVar out_p) {
  ...
  string in2 = in2_p.getPortName();
  string out = out_p.getPortName();
  string comment = ";; SMT Eq(in1, in2, out) = (" + in1 + ", " + in2 + ", " +
                   out + ")";
  ...
  string curr = "(and (=> (= " + input1 + " " + input2 + ") (= " + output +
                " #b1)) "
                "(=> (not (= " +
                input1 + " " + input2 + ")) (= " + output + " #b0)))";
  ...
  string next = "(and (=> (= " + input1 + " " + input2 + ") (= " + output +
                " #b1)) "
                "(=> (not (= " +
                input1 + " " + input2 + ")) (= " + output + " #b0)))";
...
string SMTConcat(string context, SmtBVVar in1_p, SmtBVVar in2_p,
                 SmtBVVar out_p) {
  return SMTBop(context, "Concat", "concat", in1_p, in2_p, out_p);
}

string SMTUop(string context, string opname, string op, SmtBVVar in_p,
              SmtBVVar out_p) {
  // INIT: TRUE
  ...
  string curr = unary_op_eqass(op, SMTgetCurr(context, in),
                               SMTgetCurr(context, out));
  string next = unary_op_eqass(op, SMTgetNext(context, in),
                               SMTgetNext(context, out));
  return comment + NL + curr + NL + next;
}

string SMTSlice(string context, SmtBVVar in_p, SmtBVVar out_p, int low_p,
                int high_p) {
...
string SMTReg(string context, SmtBVVar in_p, SmtBVVar clk_p, SmtBVVar out_p) {
  ...
  string out = out_p.getPortName();
  string comment = ";; SMTReg (in, clk, out) = (" + in + ", " + clk + ", " +
                   out + ")";
  string zero = getSMTbits(stoi(out_p.dimstr()), 0);
  string init = assert_op("(= " + SMTgetInit(context, out) + " " + zero + ")");
  string trans_1 = "(=> (= (bvand (bvnot " + SMTgetCurr(context, clk) + ") " +
                   SMTgetNext(context, clk) +
                   ") #b1) (= " + SMTgetNext(context, out) + " " +
                   SMTgetCurr(context, in) + "))";
  string trans_2 = "(=> (not (= (bvand (bvnot " + SMTgetCurr(context, clk) +
                   ") " + SMTgetNext(context, clk) +
                   ") #b1)) (= " + SMTgetNext(context, out) + " " +
                   SMTgetCurr(context, out) + "))";
...
// string SMTMantleReg
string SMTMantleReg(string context, Values args, SmtBVVar I_p, SmtBVVar O_p,
                    SmtBVVar CLK_p, SmtBVVar CLR_p, SmtBVVar CE_p,
                    SmtBVVar RESET_p) {
  ...
  string CLK = CLK_p.getPortName();
  string comment = ";; SMTMantleReg (I, O, CLK, CLR?, CE?, RESET?) = (" + I +
                   ", " + O + ", " + CLK + ", " + args["has_clr"]->toString() +
  ...
  string clkToggle = "(and (= " + SMTgetCurr(context, CLK) +
                     " #b0) (= " + SMTgetNext(context, CLK) + " #b1))";
  string initval = getSMTbits(width, stoi(args["init"]->toString()));
  string init = assert_op("(= " + SMTgetInit(context, O) + " " + initval + ")");
  ...
  if (has_ce && has_rst) {
    ...
    string trans2 = "(=> (and (not " + rst + ") " + clkToggle + " " + ce +
                    ") (= " + SMTgetNext(context, O) + " " +
                    SMTgetCurr(context, I) + "))";
    string trans3 = "(=> (or (and (not " + rst + ") (not " + ce + ")) (not " +
                    clkToggle + ")) (= " + SMTgetNext(context, O) + " " +
                    SMTgetCurr(context, O) + "))";
    ...
    string trans1 = "(=> " + clkToggle + " " + "(= " + SMTgetNext(context, O) +
                    " " + SMTgetCurr(context, I) + "))";
...
string SMTRegPE(string context, SmtBVVar in_p, SmtBVVar clk_p, SmtBVVar out_p,
                SmtBVVar en_p) {
  // INIT: out = 0
  ...
  string comment = ";; SMTRegPE (in, clk, out, en) = (" + in + ", " + clk +
                   ", " + out + ", " + en + ")";
  string zero = getSMTbits(stoi(out_p.dimstr()), 0);
  string init = assert_op("(= " + SMTgetInit(context, out) + " " + zero + ")");
  string trans_1 = "(=> (= (bvand " + SMTgetCurr(context, en) +
                   " (bvand (bvnot " + SMTgetCurr(context, clk) + ") " +
                   SMTgetNext(context, clk) +
                   ")) #b1) (= " + SMTgetNext(context, out) + " " +
                   SMTgetCurr(context, in) + "))";
  string trans_2 = "(=> (not (= (bvand " + SMTgetCurr(context, en) +
                   " (bvand (bvnot " + SMTgetCurr(context, clk) + ") " +
                   SMTgetNext(context, clk) +
                   ")) #b1)) (= " + SMTgetNext(context, out) + " " +
                   SMTgetCurr(context, out) + "))";
...
string SMTClock(string context, SmtBVVar clk_p) {
  ...
  string trans = assert_op("(= " + SMTgetCurr(context, clk) + " (bvnot " +
                           SMTgetNext(context, clk) + "))");
  return comment + NL + init + NL + trans;
}

string SMTMux(string context, SmtBVVar in0_p, SmtBVVar in1_p, SmtBVVar sel_p,
              SmtBVVar out_p) {
  string in0 = in0_p.getPortName();
  string in1 = in1_p.getPortName();
  string sel = sel_p.getPortName();
  string out = out_p.getPortName();
  string comment = ";; SMTMux (in0, in1, sel, out) = (" + in0 + ", " + in1 +
                   ", " + sel + ", " + out + ")";
  ...
  string curr_1 = "(=> " + then_bc + " (= " + SMTgetCurr(context, in1) + " " +
                  SMTgetCurr(context, out) + "))";
  string curr_2 = "(=> " + else_bc + " (= " + SMTgetCurr(context, in0) + " " +
                  SMTgetCurr(context, out) + "))";
  ...
  string next_1 = "(=> " + then_bn + " (= " + SMTgetNext(context, in1) + " " +
                  SMTgetNext(context, out) + "))";
  string next_2 = "(=> " + else_bn + " (= " + SMTgetNext(context, in0) + " " +
                  SMTgetNext(context, out) + "))";
...
string SMTAndr(string context, SmtBVVar in_p, SmtBVVar out_p) {
  ...
  true_res = "(=> (= " + SMTgetCurr(context, in) + " " + one + ") (" +
             SMTgetCurr(context, out) + " #b1))";
  false_res = "(=> (not (= " + SMTgetCurr(context, in) + " " + one + ")) (" +
              SMTgetCurr(context, out) + " #b0))";
  string curr = assert_op("(and " + true_res + " " + false_res + ")");

  true_res = "(=> (= " + SMTgetNext(context, in) + " " + one + ") (" +
             SMTgetNext(context, out) + " #b1))";
  false_res = "(=> (not (= " + SMTgetNext(context, in) + " " + one + ")) (" +
              SMTgetNext(context, out) + " #b0))";
...
string SMTOrr(string context, SmtBVVar in_p, SmtBVVar out_p) {
  ...
  true_res = "(=> (= " + SMTgetCurr(context, in) + " " + zero + ") (" +
             SMTgetCurr(context, out) + " #b0))";
  false_res = "(=> (not (= " + SMTgetCurr(context, in) + " " + zero + ")) (" +
              SMTgetCurr(context, out) + " #b1))";
  string curr = assert_op("(and " + true_res + " " + false_res + ")");

  true_res = "(=> (= " + SMTgetNext(context, in) + " " + zero + ") (" +
             SMTgetNext(context, out) + " #b0))";
  false_res = "(=> (not (= " + SMTgetNext(context, in) + " " + zero + ")) (" +
              SMTgetNext(context, out) + " #b1))";
  string next = assert_op("(and " + true_res + " " + false_res + ")");
