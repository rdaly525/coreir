...
string toFConst(BitVector bv) {
  return "UInt<" + to_string(bv.bitLength()) + ">(" +
    to_string(bv.to_type<uint64_t>()) + ")";
}
string toFConst(int i) { return "UInt(" + to_string(i) + ")"; }

int CoreIR::Passes::FModule::getUIntWidth(Type* t) {
  if (auto at = dyn_cast<ArrayType>(t)) {
    Type* et = at->getElemType();
    if (et->isBaseType()) { return at->getLen(); }
  }
    ...
    if (isNumber(current)) {
      ...
      if (isNumber(next)) {
        if (seenNumber) {
          ASSERT(
            0,
            "Illegal sink SelectPath (cannot multiply index in one sink "
            "expression): " +
              sp2Str(sp));
        }
...
string CoreIR::Passes::FModule::toString() {
  ...
  for (auto s : this->io) { lines.push_back("    " + s); }
  for (auto s : this->stmts) { lines.push_back("    " + s); }
...
void Passes::Firrtl::writeToStream(std::ostream& os) {
  ...
  for (auto fm : fmods) { os << fm->toString() << endl; }
}
