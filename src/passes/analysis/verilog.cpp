...
public:
  ...
  virtual std::unique_ptr<vAST::Expression>
  visit(std::unique_ptr<vAST::Expression> node) {
...
get_primitive_expr(CoreIR::Instance* instance) {
...
std::unique_ptr<vAST::StructuralStatement> inline_binary_op(
  ...
  BinaryOpReplacer transformer(
    verilog_connections->at("in0"), verilog_connections->at("in1"));
...
std::unique_ptr<vAST::Expression>
convert_to_expression(std::variant<
  ...
  for (auto instance : instances) {
    ...
    for (auto field : record_type->getFields()) {
      ...
      if (!field_type->isInput()) {
        make_wire_decl(
          instance.first + "_" + field, field_type, wire_declarations);
      }
    }
  }
...
Passes::Verilog::compilePorts(RecordType* record_type) {
  ...
  for (auto field : record_type->getFields()) {
    ...
    std::unique_ptr<vAST::Port> port = std::make_unique<vAST::Port>(
      process_decl(std::move(name), field_type), verilog_direction, vAST::WIRE);
    if (this->verilator_debug) {
  ...
  for (uint i = 0; i < select_path.size(); i++) {
    ...
    if (isNumber(item)) {
      ...
      if (isa<InstanceSelect>(curr_wireable)) {
        ...
        ASSERT(
          std::visit(
            [](auto&& node) -> bool { return node != NULL; }, curr_node),
        ...
        for (auto inst : splitString<SelectPath>(item.substr(1), ';')) {
          ...
          curr_node =
            std::make_unique<vAST::Attribute>(std::move(curr_node), inst);
        }
      }
      else {
        if (std::visit(
          ...
          ASSERT(
            std::holds_alternative<std::unique_ptr<vAST::Identifier>>(
              curr_node),
            "Expected ID");
          std::get<std::unique_ptr<vAST::Identifier>>(curr_node)->value +=
            "_" + item;
        }
      }
    }
...
void assign_module_outputs(
  ...
  for (auto field : record_type->getFields()) {
    ...
    if (entries.size() == 0) {
      ...
      process_connection_debug_metadata(
        entries[0], verilog_conn->toString(), body, field);
  ...
  for (auto instance : instances) {
    ...
    for (auto field : record_type->getFields()) {
      ...
      if (entries.size() == 0) {
        ...
        std::unique_ptr<vAST::Concat> concat =
          convert_non_bulk_connection_to_concat(
    ...
    if (can_inline_binary_op(instance_module, _inline)) {
      ...
      statement =
        inline_unary_op(instance, std::move(verilog_connections), is_wire);
    }
...
void Passes::Verilog::compileModule(Module* module) {
  ...
  if (module->hasDef()) {
    body = compile_module_body(
      module->getType(), definition, this->_inline, this->wires);
  }

  if (module->getMetaData().count("filename") > 0) {
    std::string debug_str =
      "Module `" + module->getName() + "` defined at " +
    ...
    for (auto it :
      ...
      if (metadata.count("symbol_table")) {
        while (metadata["symbol_table"].count(connect_select_path)) {
          connect_select_path =
            metadata["symbol_table"][connect_select_path].get<std::string>();
        }
      }
