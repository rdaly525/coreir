...
void* CORENewMap(
  ...
  case (STR2TYPE_ORDEREDMAP): {
    ...
    Type** types =
      (Type**)values; // TODO Sketch, this is doing an implicit rcast
...
void COREGetModArgs(
  ...
  int* num_items) {
  Values modargs =
    cast<Instance>(rcast<Wireable*>(core_wireable))->getModArgs();
...
COREModule*
CORENewModule(CORENamespace* ns, char* name, COREType* type, void* modparams) {
  Params g;
...
COREWireable*
COREModuleDefInstancesIterNext(COREModuleDef* module_def, COREWireable* curr) {
  return rcast<COREWireable*>(rcast<ModuleDef*>(module_def)
                                ->getInstancesIterNext(rcast<Instance*>(curr)));
}

COREConnection**
COREModuleDefGetConnections(COREModuleDef* m, int* numConnections) {
...
COREWireable**
COREWireableGetConnectedWireables(COREWireable* w, int* numWireables) {
...
COREGenerator*
CORENamespaceGetGenerator(CORENamespace* _namespace, const char* name) {
...
COREModule*
CORENamespaceGetModule(CORENamespace* _namespace, const char* name) {
  return rcast<COREModule*>(
...
COREDirectedConnection** COREDirectedModuleGetConnections(
  ...
  *num_connections = size;
  DirectedConnection** ptr_arr =
    module->getContext()->newDirectedConnectionPtrArray(size);
  int i = 0;
...
COREDirectedInstance** COREDirectedModuleGetInstances(
  ...
  *num_instances = size;
  DirectedInstance** ptr_arr =
    module->getContext()->newDirectedInstancePtrArray(size);
  int i = 0;
...
COREDirectedConnection** COREDirectedInstanceGetInputs(
  ...
  *num_connections = size;
  DirectedConnection** ptr_arr =
    instance->getContext()->newDirectedConnectionPtrArray(size);
  int i = 0;
  for (auto input : inputs) {
    ptr_arr[i] = input;
    i++;
