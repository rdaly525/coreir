...
CircuitPaths
buildCircuitPaths(const std::deque<vdisc>& topoOrder, NGraph& g, Module& mod) {
  CircuitPaths paths;
...
vector<SIMDGroup>
pruneSequentialSinks(const std::vector<SIMDGroup> dags, const NGraph& g) {
  vector<SIMDGroup> gp;
...
vector<SIMDGroup>
pruneOutputs(const std::vector<SIMDGroup> dags, const NGraph& g) {
  vector<SIMDGroup> gp;
...
bool nodesMatch(const vdisc ref, const vdisc a, const NGraph& g) {
  ...
  if (isInstance(rn.getWire()) && isInstance(an.getWire())) {
    ...
    if (
      ...
      if (getQualifiedOpName(*ri) == getQualifiedOpName(*ai)) {
        return true;
      }
    }
  }
...
SubDAG
alignWRT(const SubDAG& reference, const SubDAG& toAlign, const NGraph& g) {
...
alignIdenticalGraphs(const std::vector<SubDAG>& dags, const NGraph& g) {
