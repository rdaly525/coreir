...
LowExpr*
printOpResultStr(const InstanceValue& wd, const NGraph& g, LayoutPolicy& lp);
...
LowExpr*
printUnop(Instance* inst, const vdisc vd, const NGraph& g, LayoutPolicy& lp) {
...
LowExpr* printOpThenMaskBinop(
  ...
  if (isDASHR(*inst)) {
    ...
    expr =
      new LowBinop("|", new LowId(ite(signBitSet->cString(), mask, "0")), expr);
  }
...
string
printMux(Instance* inst, const vdisc vd, const NGraph& g, LayoutPolicy& lp) {
  assert(isMux(*inst));
...
void printAddOrSubCIN_COUT(
  ...
  auto sumStr = new LowBinop(
    opString, in0Str, in1Str); // parens(in0Str + opString + in1Str);
  ...
  if (standardWidth(tp)) {
    ...
    carryRes =
      parens(firstOverflow->cString() + " || " + secondOverflow->cString());
  }
  else {

    carryRes =
      parens(parens(compString + " >> " + to_string(typeWidth(tp))) + " & 0x1");
  }
...
string printSimFunctionBody(
  ...
  if (clk != nullptr) {
    ...
    LowAssign* clkUpdate =
      new LowAssign(new LowId(lastClkName), new LowId(clkName));
