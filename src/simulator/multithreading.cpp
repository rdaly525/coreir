  ...
  while (rem.size() > 0) {
    ...
    for (auto& ed : gr.inEdges(nextNode)) {
      ...
      if (cc.find(other) == end(cc)) {
        ...
        if (!isGraphInput(wd) && !isConstant(wd)) { rem.push_back(other); }
      }
    }
  ...
  for (auto& vd : gr.getVerts()) {
    ...
    if (!isGraphInput(wd) && !isConstant(wd)) {
      if (nodes.find(vd) != end(nodes)) {
        set<vdisc> ccNodes = connectedComponent(vd, gr);
...
void numberComponents(const std::vector<set<vdisc>>& ccs, NGraph& gr) {
  int numComponents = 0;
...
ThreadGraph buildThreadGraph(const NGraph& opG) {
  ...
  for (auto& v : opG.getVerts()) {
    ...
    if (!elem(threadNo, tgVerts)) { tg.addVertex(threadNo); }

    map_insert(threadComps, threadNo, v);

    if ((i % 1000) == 0) {
      cout << "Computed thread for vertex " << i
           << ", # of thread nos = " << tg.getVerts().size() << endl;
    }
  ...
  for (auto& ent : threadComps) {
    cout << "thread number " << ent.first << " contains " << ent.second.size()
         << " nodes" << endl;
    //  for (auto& vd : ent.second) {
    // cout << "\t" << vd << " = " << opG.getNode(vd).getWire()->toString() <<
    // endl;
    //  }
  }
  ...
  //   cout << "edge " << ed << " = " << opG.source(ed) << " --> " <<
  //   opG.target(ed) << endl;
  // }

  // for (auto& src : opG.getVerts()) {
  //   for (auto& dest : opG.getVerts()) {
  //  cout << src << " connected to " << dest << " ? " << opG.connected(src,
  //  dest) << endl;
  //   }
  // }
    ...
    for (uint j = 0; j < threadVerts.size(); j++) {
      if (i != j) {
        ...
        if (connectionFromTo(sourceThread, destThread, opG, threadComps)) {
          cout << "Adding edge from " << sourceThread << " to " << destThread
               << endl;
  ...
  for (auto& ed : tg.getEdges()) {
    cout << "edge " << ed << " = " << tg.source(ed) << " --> " << tg.target(ed)
         << endl;
  }
