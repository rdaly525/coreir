...
BitVector BfloatRoundToNearestEven(const BitVector& val) {
  ...
  for (int i = 0; i < 16; ++i) { bRes.set(i, val_shifted.get(i + 16)); }
...
BitVector truncateToBfloat(const BitVector& longRes) {
  ...
  // Set exponent
  for (int i = 23; i < 31; i++) { bRes.set(i - 16, longRes.get(i)); }

  for (int i = 23 - 7; i < 23; i++) { bRes.set(i - (23 - 7), longRes.get(i)); }
...
BitVector extendBfloat(const BitVector& r) {
  ...
  BitVector res(32, 0);
  for (int i = 0; i < 16; i++) { res.set(i, 0); }

  for (int i = 16; i < 23; i++) { res.set(i, mant.get(i - 16)); }

  for (int i = 23; i < 31; i++) { res.set(i, exp.get(i - 23)); }
...
int bitsToIndex(const int depth) { return ceil(log2(depth)) + 1; }
...
BitVec SimMemory::getAddr(const BitVec& bv) const {
  ...
  if (it == std::end(values)) { return BitVec(width, 0); }
...
std::vector<CircuitState> SimulatorState::getCircStates() const {
  return circStates;
}

int SimulatorState::numCircStates() const { return circStates.size(); }

int SimulatorState::getStateIndex() const { return stateIndex; }
  ...
  for (auto& vd : gr.getVerts()) {
    ...
    if (isMemoryInstance(wd.getWire())) {
      ...
      if (contains_key(string("init"), params)) {
        ...
        // cout << "Memory node " << inst->getInstname() << " has init params"
        // << endl; cout << "Its params size = " << params.size()  << endl; for
        // (auto p : params) { cout << "\t" << p.first << endl;
        //}
...
void SimulatorState::setMainClock(CoreIR::Select* s) { mainClock = s; }
...
void SimulatorState::setWatchPoint(const std::string& val, const BitVec& bv) {

  StopFunction func = [this, val, bv]() {
    if (exists(val)) {
...
bool SimulatorState::isSet(CoreIR::Select* s) const {
  if (!valMapContains(s)) {
    ...
    if (isNumber(str)) { return isSet(toSelect(s->getParent())); }
...
void SimulatorState::stepMainClock() { stepClock(mainClock); }

void SimulatorState::run() {
  while (!hitWatchPoint()) { runHalfCycle(); }
}

void SimulatorState::runBack() {
  while (!hitWatchPoint()) { rewind(1); }
}
...
void SimulatorState::findMainClock() {
  ...
  if (clkInput != nullptr) { setMainClock(clkInput); }
}
...
void SimulatorState::initializeState(
  ...
  if (mod->getMetaData().get<map<string, json>>().count("symtable")) {
    hasSymTable = true;
    symTable = mod->getMetaData()["symtable"].get<map<string, json>>();
  }
  ...
  for (auto vdPair : rcvToSrc) {
    ...
    SimulatorPlugin* plugin = pluginBuilders[instName](wd);
...
bool isSimBitVector(SimValue* v) { return v->getType() == SIM_VALUE_BV; }
...
SimValue* SimulatorState::getValue(CoreIR::Select* sel) {
  ...
  if (it == std::end(circStates[stateIndex].valMap)) { assert(false); }
...
void SimulatorState::updateSliceNode(const vdisc vd) {
  ...
  for (uint i = lo; i < hi; i++) { res.set(i - lo, sB.get(i)); }
...
void SimulatorState::updateAndNode(const vdisc vd) {
  updateBitVecBinop(vd, [](const BitVec& l, const BitVec& r) { return l & r; });
}
...
void SimulatorState::updateConcatNode(const vdisc vd) {
  updateBitVecBinop(vd, [](const BitVec& s0Bits, const BitVec& s1Bits) {
    ...
    for (int i = 0; i < s0Bits.bitLength(); i++) { conc.set(i, s0Bits.get(i)); }
...
void SimulatorState::updateDFFValue(const vdisc vd) { updateRegisterValue(vd); }
...
void SimulatorState::updateMemoryValue(const vdisc vd) {
  ...
  // SimBitVector* waddr =
  // static_cast<SimBitVector*>(getValue(waddrV.getWire())); SimBitVector* wdata
  // = static_cast<SimBitVector*>(getValue(wdataV.getWire())); SimBitVector* wen
  // = static_cast<SimBitVector*>(getValue(enArg.getWire())); ClockValue* clkVal
  // = toClock(getValue(clkArg.getWire()));
...
BitVec SimulatorState::getMemory(const std::string& name, const BitVec& addr) {
...
void SimulatorState::resetCircuit() { exeCombinational(); }
...
void SimulatorState::exeSequential() {
  ...
  for (auto& vd : gr.getVerts()) {
    ...
    if (isMemoryInstance(wd.getWire())) {
      if (wd.isReceiver) { updateMemoryValue(vd); }
    }

    if (isDFFInstance(wd.getWire()) && wd.isReceiver) { updateDFFValue(vd); }
  }
}
  ...
  for (uint i = 0; i < 2; ++i) {
    ...
    if (!hasCombinationalLoop) {
      ...
      for (auto& vd : gr.getVerts()) {
        ...
        if (isGraphInput(w)) { freshNodes.insert(vd); }
      }
...
void SimulatorState::execute() {
  ...
  if (unsetIns.size() > 0) {
    cout << "Cannot execute because " << unsetIns.size()
         << " input(s) are not set:" << endl;
    for (auto& vd : unsetIns) {
      cout << "\t" << getCircuitGraph().getNode(vd).getWire()->toString()
           << endl;
    }
    return;
  }
...
void SimulatorState::setRegister(const std::string& name, const BitVec& data) {
...
bool SimulatorState::valMapContains(CoreIR::Select* sel) const {
  return circStates[stateIndex].valMap.find(sel) !=
         std::end(circStates[stateIndex].valMap);
}
...
bool isPrefixOf(const std::string& foo, const std::string& foobar) {
  auto res = std::mismatch(foo.begin(), foo.end(), foobar.begin());

  if (res.first == foo.end()) { return true; }
...
std::vector<string> selectsOffOf(
  ...
  for (auto& ent : symTable) {
    ...
    if (isPrefixOf(selectName, selName)) { sels.push_back(selName); }
  }

  return sels;
}

SimValue* SimulatorState::getValueByOriginalName(const std::string& name) {
  ...
  vector<string> postFixes = selectsOffOf(name, symTable);
...
SimulatorState::~SimulatorState() {
  for (auto& val : allocatedValues) { delete val; }

  set<SimulatorPlugin*> plugs;
  for (auto pg : plugMods) { plugs.insert(pg.second); }
