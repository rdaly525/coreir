...
void writeBitVectorLib() { writeBitVectorLib("./gencode/bit_vector.h"); }
...
int compileCode(
  ...
  string runCmd = "clang++ -march=native -lpthread -std=c++11 -c " +
    codeFilePath;
  int s = system(runCmd.c_str());
...
int compileCodeAndRun(
  ...
  string runCmd = "clang++ -march=native -lpthread -std=c++11 " + codeFilePath +
    " " + harnessFilePath;
  int s = system(runCmd.c_str());
...
int compileCodeAndRun(
  ...
  string runCmd = "clang++ -march=native -lpthread -std=c++11 " + outFile +
    " " + harnessFile;
  int s = system(runCmd.c_str());
