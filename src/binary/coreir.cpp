...
int main(int argc, char* argv[]) {
  ...
  if (opts.count("l")) {
    vector<string> libs =
      splitString<vector<string>>(opts["l"].as<string>(), ',');
    for (auto lib : libs) {
  ...
  if (opts.count("e")) {
    vector<string> passes =
      splitString<vector<string>>(opts["e"].as<string>(), ',');
  ...
  if (outExt == "json") {
    ...
    modified |=
      c->runPasses({"removebulkconnections", "flattentypes", "smtlib2"});
    ...
    auto vpass =
      static_cast<Passes::SMV*>(c->getPassManager()->getAnalysisPass("smv"));
