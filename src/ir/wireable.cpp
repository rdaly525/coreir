...
bool Wireable::canSel(string selstr) { return type->canSel(selstr); }

bool Wireable::canSel(SelectPath path) { return type->canSel(path); }
...
void Wireable::connect(Wireable* w) { this->getContainer()->connect(this, w); }

void Wireable::disconnect() { this->getContainer()->disconnect(this); }

void Wireable::disconnectAll() {
  for (auto sels : this->getSelects()) { sels.second->disconnectAll(); }
  this->disconnect();
}
...
void Wireable::reconnect(Wireable* other) {
  ...
  for (auto from_connected : this->getConnectedWireables()) {
    other->connect(from_connected);
  }
  disconnect();
  for (auto &[field, from_subfield] : this->getSelects()) {
...
LocalConnections Wireable::getLocalConnections() {
  ...
  traverse = [&cons, &traverse, &uniqueCons](Wireable* curw) -> void {
    ...
    for (auto sels : curw->getSelects()) { traverse(sels.second); }
  };
...
string Instance::toString() const { return instname; }
