...
template <typename T> ArgPtr Const_impl2(T val) {
  return std::make_shared<typename Val2Arg<T>::type>(val);
}

template <> ArgPtr Const_impl<bool>(bool val) { return Const_impl2<bool>(val); }

template <> ArgPtr Const_impl<int>(int val) { return Const_impl2<int>(val); }

template <> ArgPtr Const_impl<std::string>(std::string val) {
  return Const_impl2<std::string>(val);
}

template <> ArgPtr Const_impl<Type*>(Type* val) {
  return Const_impl2<Type*>(val);
}

bool Arg::operator==(const Arg& r) const { return r.getKind() == this->kind; }
