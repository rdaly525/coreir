...
std::string ValueType::toString() {
  ...
  case VTK_Int:
    return "Int";
  case VTK_BitVector:
    return "BitVector<" + to_string(cast<BitVectorType>(this)->getWidth()) +
      ">";
  case VTK_String:
    return "String";
  case VTK_CoreIRType:
...
BitVectorType* BitVectorType::make(Context* c, int width) {
  return c->typecache->getBitVector(width);
}
StringType* StringType::make(Context* c) { return c->typecache->getString(); }
CoreIRType* CoreIRType::make(Context* c) {
  return c->typecache->getCoreIRType();
}
ModuleType* ModuleType::make(Context* c) {
  return c->typecache->getModuleType();
}
