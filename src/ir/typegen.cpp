...
Type* TypeGen::getType(Values args) {
  if (typeCache.count(args)) { return typeCache.at(args); }
  checkValuesAreParams(args, params);
  try {
    Type* t = this->createType(args);
    assert(t);
    if (flipped) { t = t->getFlipped(); }
    typeCache[args] = t;
    return t;
  } catch (const std::out_of_range&) {
    cout << "Failed on " << this->getRefName()
         << " with args=" << ::CoreIR::toString(args) << endl;
    assert(0);
    return nullptr;
  }
}
...
TypeGenFromFun* TypeGenFromFun::make(Namespace* ns, std::string name,
                                     Params genparams, TypeGenFun fun,
                                     bool flipped) {
...
TypeGenSparse::TypeGenSparse(Namespace* ns, std::string name, Params params,
                             std::vector<std::pair<Values, Type*>>& typelist)
    : TypeGen(ns, name, params, false) {
  for (auto vpair : typelist) {
    ASSERT(typeLookup.count(vpair.first) == 0,
           "In " + this->toString() + " Cannot add duplicate " +
               ::CoreIR::toString(vpair.first));
...
Type* TypeGenSparse::createType(Values genargs) {
  if (typeLookup.count(genargs)) { return typeLookup[genargs]; }
  ASSERT(0, "Typegen: " + this->toString() +
                " cannot handle args=" + ::CoreIR::toString(genargs));
}
...
TypeGenSparse* TypeGenSparse::make(
...
bool TypeGenImplicit::hasType(Values genargs) { return false; }
...
TypeGenImplicit* TypeGenImplicit::make(Namespace* ns, std::string name,
                                       Params params) {
