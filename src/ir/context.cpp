...
Context::~Context() {
  delete pm;
  for (auto it : recordParamsList)
    delete it;
  for (auto it : paramsList)
    delete it;
  for (auto it : connectionPtrArrays)
    free(it);
  for (auto it : connectionArrays)
    free(it);
  for (auto it : wireableArrays)
    free(it);
  for (auto it : constStringArrays)
    free(it);
  for (auto it : stringArrays)
    free(it);
  for (auto it : stringBuffers)
    free(it);
  for (auto it : directedConnectionPtrArrays)
    free(it);
  for (auto it : directedInstancePtrArrays)
    free(it);
  for (auto it : valuePtrArrays)
    free(it);
  for (auto it : valueTypePtrArrays)
    free(it);
...
// This tries to link all the definitions of def namespace to declarations of
decl namespace
...
bool Context::linkLib(Namespace* nsFrom, Namespace* nsTo) {
  ...
  for (auto it : defns->getGenerators()) {
    ...
    if (gdeffun && gdeclfun && (gdeffun != gdeclfun) ) {
      Error e;
      e.message("Cannot link a def if there is already a def! (duplicate
symbol)"); e.message("  Cannot link : " + defns->getName() + "." + gdefname);
...
RecordType* Context::Record(RecordParams rp) {
  return typecache->getRecord(rp);
}
...
BitVectorType* Context::BitVector(int width) {
  return BitVectorType::make(this, width);
}
...
DirectedConnection** Context::newDirectedConnectionPtrArray(int size) {
  DirectedConnection** arr =
    (DirectedConnection**)malloc(sizeof(DirectedConnection*) * size);
  directedConnectionPtrArrays.push_back(arr);
  return arr;
}

DirectedInstance** Context::newDirectedInstancePtrArray(int size) {
  DirectedInstance** arr =
    (DirectedInstance**)malloc(sizeof(DirectedInstance*) * size);
...
void deleteContext(Context* m) { delete m; }
