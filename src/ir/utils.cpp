...
bool isBitArray(Type& t) {
  ...
  if (t.getKind() != Type::TK_Array) { return false; }

  ArrayType& tArr = static_cast<ArrayType&>(t);

  Type::TypeKind elemKind = (tArr.getElemType())->getKind();
  if ((elemKind == Type::TK_Bit || elemKind == Type::TK_BitIn)) { return true; }
...
bool isPrimitiveType(Type& t) {
  ...
  if ((t.getKind() == Type::TK_BitIn) || (t.getKind() == Type::TK_Bit)) {
    return true;
  }

  if (isBitArrayOfLengthLEQ(t, 8)) { return true; }

  if (isBitArrayOfLengthLEQ(t, 16)) { return true; }

  if (isBitArrayOfLengthLEQ(t, 32)) { return true; }

  if (isBitArrayOfLengthLEQ(t, 64)) { return true; }

  return false;
}

unordered_map<string, Wireable*> getOutputSelects(Wireable* inst) {
  unordered_map<string, Wireable*> outs;

  for (auto& select : inst->getSelects()) {
    if (select.second->getType()->isOutput()) { outs.insert(select); }
  }

  return outs;
}

unordered_map<string, Wireable*> getInputSelects(Wireable* inst) {
  unordered_map<string, Wireable*> outs;
...
bool recordTypeHasField(const std::string& fieldName, Type* t) {
  ...
  for (auto& rec : rt->getRecord()) {
    if (rec.first == fieldName) { return true; }
  }
...
uint typeWidth(CoreIR::Type& tp) {
  ...
  if ((tp.getKind() == Type::TK_BitIn) || (tp.getKind() == Type::TK_Bit)) {
    return 1;
  }
...
std::unordered_map<string, Type*> outputs(Module& mod) {
  Type* tp = mod.getType();
  ...
  for (auto& name_type_pair : modRec->getRecord()) {
    ...
    if (tp->isOutput()) { outs.insert(name_type_pair); }
  }
...
BitVector hexStringToBitVector(const std::string& str) {
  ...
  for (auto byte : addrBytes) {
    ...
    for (uint i = 0; i < (uint)tmp.bitLength(); i++) {
      ...
      if (offset < numBits) { configAddr.set(offset, tmp.get(i)); }
      offset++;
    }
  }
...
BitStreamConfig loadConfig(const std::string& configFileName) {
  ...
  for (auto line : configLines) { cout << "\t" << line << endl; }
