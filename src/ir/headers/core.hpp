...
void core_convert(Context* c, Namespace* core) {
  ...
  auto sliceTypeGen =
    core->newTypeGen("sliceTypeFun", sliceParams, [](Context* c, Values args) {
  ...
  auto extTypeGen =
    core->newTypeGen("extTypeFun", extParams, [](Context* c, Values args) {
  ...
  auto stripTypeGen =
    core->newTypeGen("stripTypeFun", stripParams, [](Context* c, Values args) {
  ...
  auto wrapTypeGen =
    core->newTypeGen("wrapTypeFun", wrapParams, [](Context* c, Values args) {
      Type* type = args.at("type")->get<Type*>();
      ASSERT(isa<NamedType>(type), "type needs to be a named type");
      NamedType* ntype = cast<NamedType>(type);
      ASSERT(!ntype->isGen(), "NYI named type generators");
...
void core_state(Context* c, Namespace* core) {
  ...
  TypeGen* regRstTypeGen =
    core->newTypeGen("regRstType", widthparams, regRstFun);
...
Namespace* CoreIRLoadHeader_core(Context* c) {
  ...
  core->newTypeGen("unary", widthparams, [](Context* c, Values args) {
    uint width = args.at("width")->get<int>();
    Type* ptype = c->Bit()->Arr(width);
    return c->Record({{"in", c->Flip(ptype)}, {"out", ptype}});
  });
  core->newTypeGen("binary", widthparams, [](Context* c, Values args) {
    uint width = args.at("width")->get<int>();
    Type* ptype = c->Bit()->Arr(width);
    return c->Record(
  ...
  // For mux
  core->newTypeGen("muxType", widthparams, [](Context* c, Values args) {
    uint width = args.at("width")->get<int>();
    Type* ptype = c->Bit()->Arr(width);
  ...
  TypeGen* tribufTG =
    core->newTypeGen("triBuf", widthparams, [](Context* c, Values args) {
      uint width = args.at("width")->get<int>();
  ...
  TypeGen* ibufTG =
    core->newTypeGen("iBuf", widthparams, [](Context* c, Values args) {
      uint width = args.at("width")->get<int>();
      return c->Record(
  ...
  core->newTypeGen("pullResistor", widthparams, [](Context* c, Values args) {
    ...
    return c->Record({{"out", c->BitInOut()->Arr(width)}});
  });
  ...
  auto Const =
    core->newGeneratorDecl("const", core->getTypeGen("out"), widthparams);
