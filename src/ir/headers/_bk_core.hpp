...
void coreirprims_state(Context* c, Namespace* coreirprims) {
  ...
  Params regGenParams({{"width", c->Int()},
                       ...
                       {"rst", c->Bool()}});
  TypeGen* regTypeGen =
    coreirprims->newTypeGen("regType", regGenParams, regFun);
  ...
  TypeGen* bitRegTypeGen =
    coreirprims->newTypeGen("bitRegType", bitRegGenParams, bitRegFun);
  auto bitreg =
    coreirprims->newGeneratorDecl("bitreg", bitRegTypeGen, bitRegGenParams);
  ...
  TypeGen* memTypeGen =
    coreirprims->newTypeGen("memType", memGenParams, memFun);
...
Namespace* CoreIRLoadLibrary_coreirprims(Context* c) {
  ...
  for (auto op : {"add", "sub"}) {
    Generator* genDecl =
      coreirprims->newGeneratorDecl(op, binaryCarryTypeGen, binaryCarryParams);
  ...
  // Do The
  Type* bitBinaryType =
    c->Record({{"in0", c->BitIn()}, {"in1", c->BitIn()}, {"out", c->Bit()}});
  ...
  coreirprims->newModuleDecl("bitnot", bitUnaryType)->getMetaData()["verilog"] =
    bverilog;
