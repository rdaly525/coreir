...
TypeCache::~TypeCache() {
  ...
  // TODO this is a little sketch because the first key is the thing you are
  // deleting...
  for (auto tpair : ArrayCache) {
    for (auto ttpair : tpair.second) { delete ttpair.second; }
  }

  for (auto bpair : bitVectorCache) { delete bpair.second; }
...
RecordType* TypeCache::getRecord(RecordParams params) {
  ...
  if (it != RecordCache.end()) { return it->second; }
    ...
    for (auto p : params) { paramsF.push_back({p.first, c->Flip(p.second)}); }
