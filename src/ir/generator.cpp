...
Generator::~Generator() {
  delete def;
  for (auto m : genCache) { delete  .second; }
}

// This is the tough one
Module* Generator::getModule(Values genargs) {
  mergeValues(genargs, defaultGenArgs);
  if (genCache.count(genargs)) { return genCache[genargs]; }
...
bool Generator::runAll() {
  bool ret = false;
  for (auto mpair : genCache) { ret |= mpair.second->runGenerator(); }
  return ret;
}
...
void Generator::print(void) const { cout << toString() << endl; }
