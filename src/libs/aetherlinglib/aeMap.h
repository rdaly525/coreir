...
vector<pair<string, Type*>> getInputOrOutputFields(
  ...
  for (auto& field : fields) {
    ...
    if (field.first == "ready" || field.first == "valid") { continue; }
...
void Aetherling_createMapGenerator(Context* c) {
  ...
  mapParallel->setGeneratorDefFromFun(
    [](Context* c, Values genargs, ModuleDef* def) {
      ...
      for (uint i = 0; i < numInputs; i++) {
        ...
        for (auto opInputField : opInputFields) {
          def->connect(
            "self." + opInputField.first + "." + idxStr,
            opStr + "." + opInputField.first);
        }
        for (auto opOutputField : opOutputFields) {
          def->connect(
            opStr + "." + opOutputField.first,
            "self." + opOutputField.first + "." + idxStr);
        }
      }
    });
  ...
  /*
   * This implementation of map is fully sequential, takes entire input in first
