...
/*
 * Since same process for dehydrate and hydrate, just wiring in different
 * direction, reusing logic
 */
void walkTypeTree(
  ModuleDef* def,
  uint dehydratedSize,
  ...
  for (int curDehydratedIndex = 0; !elementsToExamine.empty();) {
    ...
    // add its children elements to the queue if its not a bit, or wire up the
    // bits
    switch (curType->getKind()) {
    case Type::TypeKind::TK_Bit:  // if non-array of bits type is output,
                                  // hydrate as hydrate
      ...
      break;
    case Type::TypeKind::TK_BitIn:  // if non-array of bits type is input,
                                    // dehydrate as converting any type
      // to array of bits
    ...
    case Type::TypeKind::TK_Array: {
      ...
      for (uint i = 0; i < curAsArr->getLen(); i++) {
        elementsToExamine.push(
          make_tuple(elemType, curName + "." + to_string(i)));
      }
      break;
    }
    case Type::TypeKind::TK_Record: {
      ...
      for (auto it = nameTypeMap.begin(); it != nameTypeMap.end(); ++it) {
        elementsToExamine.push(
          make_tuple(it->second, curName + "." + it->first));
      }
      break;
    }
    ...
    default:
      ASSERT(0, "hydrating or dehydrating invalid type " + curType->toString());
    }
  }
}

/* Dehydrate converts a nested structure into an array of bits that can be
 * passed to muxes, serializers, and other objects that can't handle complex
 * types Note: unlike flatten, don't need get size as all types already have a
 * getsize that gives number of bits
 */
...
void Aetherling_createHydrateAndDehydrateGenerators(Context* c) {
   ...
   */
  Params hydrateParams = Params({{"hydratedType", CoreIRType::make(c)}});
  ...
  aetherlinglib->newTypeGen(
    ...
    [](Context* c, Values genargs) {  // Function to compute type
      ...
      return c->Record(
        {{"in", c->In(inputType)}, {"out", c->Bit()->Arr(outputSize)}});
    });
  ...
  dehydrate->setGeneratorDefFromFun(
    [](Context* c, Values genargs, ModuleDef* def) {
      ...
      uint outputSize = inputType->getSize();
      // note: this gets a value for each element, so if an array has n
      // elements, this gets pushed n times the string is the name of the type
      queue<tuple<Type*, string>> elementsToExamine;
      // first element to examine is hydrated input and its name relative to
      // root
  ...
  aetherlinglib->newTypeGen(
    ...
    [](Context* c, Values genargs) {  // Function to compute type
      ...
      return c->Record(
        {{"in", c->BitIn()->Arr(inputSize)}, {"out", c->Out(outputType)}});
    });
  ...
  hydrate->setGeneratorDefFromFun(
    [](Context* c, Values genargs, ModuleDef* def) {
      ...
      // elements, this gets pushed n times the string is the name of the type
      queue<tuple<Type*, string>> elementsToExamine;
      // first element to examine is hydrated output and its name relative to
      // root
