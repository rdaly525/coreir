...
Namespace* CoreIRLoadLibrary_rtlil(CoreIR::Context* c) {
  ...
  for (auto& name : rtlilBinaryComps) {
    ...
    std::function<void(Context*, Values, ModuleDef*)> genFun =
      [name](Context* c, Values args, ModuleDef* def) {
        ...
        if (a_signed && b_signed) { bothSigned = true; }
        else {
          if (a_signed || b_signed) {
            cout << "operation = " << name << endl;
            cout << "a_signed = " << a_signed << endl;
        ...
        string opGenName;
        if (bothSigned) { opGenName = rtlilSignedComparatorName(name); }
        else {
  ...
  TypeGen* outToInOutTP = rtLib->newTypeGen(
    "outArrayToInOutArray", outToInOutParams, [](Context* c, Values genargs) {
      ...
      return c->Record(
        {{"IN", c->BitIn()->Arr(width)}, {"OUT", c->BitInOut()->Arr(width)}});
    });
