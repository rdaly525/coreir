...
std::string rtlilSignedComparatorName(const std::string& name) {
  if (name == "lt") { return "coreir.slt"; }

  if (name == "gt") { return "coreir.sgt"; }

  if (name == "ge") { return "coreir.sge"; }

  if (name == "le") { return "coreir.sle"; }

  if (name == "eq") { return "coreir.eq"; }

  if (name == "ne") { return "coreir.neq"; }
...
std::string rtlilCorebitName(const std::string& name) {
  if (name == "and") { return "corebit.and"; }

  if (name == "or") { return "corebit.or"; }

  if (name == "xor") { return "corebit.xor"; }
...
std::string rtlilCoreirName(const std::string& name) {
  ...
  if (name == "shl") { return "coreir.shl"; }

  if (name == "shr") { return "coreir.lshr"; }

  if (name == "sshr") { return "coreir.ashr"; }

  if (name == "not") { return "coreir.not"; }

  if (name == "reduce_or") { return "coreir.orr"; }

  if (name == "reduce_bool") { return "coreir.orr"; }

  if (name == "reduce_and") { return "coreir.andr"; }

  if (name == "reduce_xor") { return "coreir.xorr"; }

  if (name == "add") { return "coreir.add"; }

  if (name == "sub") { return "coreir.sub"; }

  if (name == "mul") { return "coreir.mul"; }

  if (name == "and") { return "coreir.and"; }

  if (name == "or") { return "coreir.or"; }

  if (name == "xor") { return "coreir.xor"; }
  ...
  if (name == "ge") { return "coreir.uge"; }

  if (name == "le") { return "coreir.ule"; }

  if (name == "gt") { return "coreir.ugt"; }

  if (name == "lt") { return "coreir.ult"; }
...
Namespace* CoreIRLoadLibrary_rtlil(CoreIR::Context* c) {
  ...
  vector<string> rtlilBinops{
      "and",       "or",       "xor", "xnor", "shl", "shr", "sshl", "sshr",
      "logic_and", "logic_or", "eqx", "nex",  "lt",  "le",  "eq",   "ne",
      "ge",        "gt",       "add", "sub",  "mul", "div", "mod",  "pow"};
  ...
  for (auto& name : rtlilBinops) {
    ...
    TypeGen* logic_andTP = rtLib->newTypeGen(
        name, binopParams, [](Context* c, Values genargs) {
          ...
          return c->Record({{"A", c->BitIn()->Arr(a_width)},
  ...
  for (auto& name : rtlilShifts) {
    ...
    std::function<void(Context*, Values, ModuleDef*)> genFun =
        [name](Context* c, Values args, ModuleDef* def) {
          ...
          // Does it matter whether a shift operand is signed or not? sshr vs
          // shr is what determines whether to zero extend or not right? bool
          // a_signed = args.at("A_SIGNED")->get<bool>(); bool b_signed =
          // args.at("B_SIGNED")->get<bool>();

          // ASSERT(!a_signed, "Have not yet added signed input support for " +
          // name); ASSERT(!b_signed, "Have not yet added signed shift value
          // support for " + name);

          ASSERT(y_width >= a_width,
                 "Shift operations must have output at least as long as bit "
                 "vector being shifted");

          uint res_width = max(a_width, y_width);
          uint ext_width = max(res_width, b_width);

          def->addInstance("extendA", "coreir.zext",
                           {{"width_in", Const::make(c, a_width)},
                            {"width_out", Const::make(c, ext_width)}});

          def->addInstance("extendB", "coreir.zext",
  ...
  for (auto& name : rtlilBitwise) {
    ...
    std::function<void(Context*, Values, ModuleDef*)> genFun =
        [name](Context* c, Values args, ModuleDef* def) {
          ...
          ASSERT(!signsMatter || (!a_signed && !b_signed),
                 "Have not yet added signed arithmetic support for RTLIL");

          // ASSERT(!a_signed, "Have not yet added signed arithmetic support for
          // RTLIL"); ASSERT(!b_signed, "Have not yet added signed arithmetic
          // support for RTLIL");

          ASSERT(y_width >= a_width,
                 "Bitwise and arithmetic operations must have output at least "
                 "as long as operands");
          ASSERT(y_width >= b_width,
                 "Bitwise and arithmetic operations must have output at least "
                 "as long as operands");
  ...
  // - Comparators
  vector<string> rtlilBinaryComps{"eqx", "nex", "lt", "le",
                                  "eq",  "ne",  "ge", "gt"};
    ...
    std::function<void(Context*, Values, ModuleDef*)> genFun =
        [name](Context* c, Values args, ModuleDef* def) {
          ...
          if (a_signed && b_signed) {
            ...
            ASSERT(!a_signed,
                   "Have not yet added signed comparator support for RTLIL");
            ASSERT(!b_signed,
                   "Have not yet added signed comparator support for RTLIL");
          }
  ...
  std::function<void(Context*, Values, ModuleDef*)> logicOrGenFun =
      [](Context* c, Values args, ModuleDef* def) {
        ...
        ASSERT(!a_signed,
               "Have not yet added signed logic_or support for RTLIL");
        ASSERT(!b_signed,
               "Have not yet added signed logic_or support for RTLIL");
        ...
        def->addInstance("aRed", "coreir.orr",
                         {{"width", Const::make(c, ext_width)}});
        def->addInstance("bRed", "coreir.orr",
                         {{"width", Const::make(c, ext_width)}});
  ...
  vector<string> rtlilUnops{"not",         "pos",         "neg",
  ...
  for (auto& name : rtlilUnops) {
    ...
    TypeGen* logic_andTP = rtLib->newTypeGen(
        name, binopParams, [](Context* c, Values genargs) {
          ...
          return c->Record(
              {{"A", c->BitIn()->Arr(a_width)}, {"Y", c->Bit()->Arr(y_width)}});
        });
  ...
  std::function<void(Context*, Values, ModuleDef*)> genFun = [](Context* c,
                                                                Values args,
                                                                ModuleDef*
                                                                    def) {
    uint a_width = args.at("A_WIDTH")->get<int>();
    ...
    ASSERT(y_width >= a_width,
           "Output of not must be at least as large as its input");
  ...
  std::function<void(Context*, Values, ModuleDef*)> lnotGenFun =
      [](Context* c, Values args, ModuleDef* def) {
        ...
        def->addInstance("reduce", "coreir.orr",
        ...
        // string opGenName = rtlilCoreirName("not");
        // def->addInstance("op0", opGenName, {{"width", Const::make(c,
        // y_width)}});
  ...
  for (auto& name : rtlilReduceOps) {
    ...
    std::function<void(Context*, Values, ModuleDef*)> genFun = [name](
                                                                   Context* c,
                                                                   Values args,
                                                                   ModuleDef*
                                                                       def) {
      ...
      // ASSERT(!a_signed, "Have not yet added signed reduce support for
      // RTLIL");
  ...
  muxGen->setGeneratorDefFromFun([](Context* c, Values args, ModuleDef* def) {
    ...
    // if (width > 1) {
    mux = def->addInstance("mux0", "coreir.mux",
  ...
  dffGen->setGeneratorDefFromFun([](Context* c, Values args, ModuleDef* def) {
    ...
    reg = def->addInstance("reg0", "coreir.reg",
                           {{"width", Const::make(c, width)}});
    ...
    def->addInstance("toClk0", "coreir.wrap",
  ...
  auto adffModParamFun = [](Context* c,
  ...
  adffGen->setGeneratorDefFromFun([](Context* c, Values args, ModuleDef* def) {
    ...
    reg = def->addInstance("reg0", "coreir.reg_arst",
                           {{"width", Const::make(c, width)}},
                           {{"init", initVal},
    ...
    def->addInstance("toRST0", "coreir.wrap",
  ...
  rtLib->newGeneratorDecl("outArrayToInOutArray", outToInOutTP,
                          outToInOutParams);
  ...
  rtLib->newGeneratorDecl("inOutArrayToOutArray", inOutToOutTP,
                          inOutToOutParams);
  ...
  Type* highImpedanceType = c->Record({{"OUT", c->Bit()}});
  rtLib->newModuleDecl("highImpedanceBit", highImpedanceType);

  Type* unknownBitType = c->Record({{"OUT", c->Bit()}});
  auto uMod = rtLib->newModuleDecl("unknownBit", unknownBitType);
  auto uDef = uMod->newModuleDef();

  uDef->addInstance("uConst", "coreir.const", {{"width", Const::make(c, 1)}},
