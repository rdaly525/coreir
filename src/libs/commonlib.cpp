...
vector<CoreIR::Wireable*>
get_wires(CoreIR::Wireable* base_wire, const vector<size_t> ports) {
  int num_ports = 1;
...
Namespace* CoreIRLoadLibrary_commonlib(Context* c) {
  ...
  Generator* const_array =
    commonlib->newGeneratorDecl("const_array", constArrayTG, const_array_args);
  ...
  Generator* reg_array =
    commonlib->newGeneratorDecl("reg_array", regArrayTG, reg_array_args);
  ...
  reg_array->setGeneratorDefFromFun(
    [](Context* c, Values args, ModuleDef* def) {
      ...
      for (uint i = 0; i < in_wires.size(); ++i) {
        ...
        Wireable* reg =
          def->addInstance(reg_name, "mantle.reg", reg_args, reg_configargs);
        if (en) {
          def->connect("self.en", reg_name + ".en");
        }
        if (clr) {
          def->connect("self.clr", reg_name + ".clr");
        }
        if (rst) {
          def->connect("self.rst", reg_name + ".rst");
        }
  ...
  commonlib->newTypeGen(
    ...
    [](Context* c, Values args) {              // Function to compute type
      ...
      return c->Record({{"en", c->BitIn()},
                        ...
                        {"ready", c->Bit()}, // have cycled through all outputs,
                                             // put new inputs on this cycle
