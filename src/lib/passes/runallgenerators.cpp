...
class runAllGeneratorsPass : public InstanceDAGPass {
  public :
    runAllGeneratorsPass() : InstanceDAGPass() {}
    bool runOnModule(Module* m) {
      return false;
    } //Need to run recursively on instances
    bool runOnInstance(Instance* i) {
      bool changed = false;
      if (i->isGen()) {
        changed |= i->runGenerator();
        if (changed) {
          for (auto instmap : i->getModuleRef()) {
            this->runOnInstance(instmap.second);
          }
        }
      }
      return changed;
