...
class LayoutPolicy {
 ...
 public:
  ...
  virtual bool getReadRegsDirectly() const { return readRegsDirectly; }
...
static inline CoreIR::Select* baseSelect(CoreIR::Select* const sel) {
  if (!isSelect(sel->getParent())) { return sel; }
 ...
 public:
  ...
  bool adjacent(
    ...
    for (uint i = 0; i < decls.size() - 1; i++) {
      if ((decls[i].second == a) && (decls[i + 1].second == b)) { return true; }
    }

    return false;
  }

  bool canMerge(const std::string& a, const std::string& b) {
    if (adjacentGroups.size() == 0) { return true; }
    ...
    for (uint i = 0; i < adjacentGroups.size(); i++) {
      ...
      for (uint j = 0; j < valGroup.size(); j++) {
        if (valGroup[j].second == a) { aGroup = i; }

        if (valGroup[j].second == b) { bGroup = i; }
      }
    }
    ...
    if (
      (adjacentGroups[aGroup].back().second == a) &&
  ...
  void forceAdjacent(const std::vector<std::string>& vars) {
    ...
    for (auto& ind : adjacentInds) { adj.push_back(varDecls[ind]); }

    std::vector<std::pair<Type*, std::string>> others;
    for (uint i = 0; i < varDecls.size(); i++) {
      if (!elem(i, adjacentInds)) { others.push_back(varDecls[i]); }
    }
    ...
    ASSERT(
      varDecls.size() == ((unsigned)oldSize),
      "oldsize is " + std::to_string(oldSize));
  }
  ...
  std::string lastClkVarName(InstanceValue& clk) {
    if (!elem(cVar("", clk, "_last"), allocatedAlready)) {
      ...
      adjacentGroups.push_back(
        {{clk.getWire()->getType(), cVar("", clk, "_last")}});
  ...
  std::string outputVarName(CoreIR::Wireable& val) {
    ...
    if (isSelect(val)) { return selectName(toSelect(&val)); }
  ...
  std::string outputVarName(const InstanceValue& val) {
    std::cout << "Creating output for " << val.getWire()->toString()
              << std::endl;
