...
std::vector<Select*> allOutputSelects(CoreIR::Wireable* inst) {
  vector<Select*> conns;
...
std::vector<Select*> allInputSelects(CoreIR::Wireable* inst) {
  vector<Select*> conns;
...
std::vector<CoreIR::Instance*> receiverInstances(
  ...
  if (isa<Select>(w)) {
    ...
    if (wSel->getType()->getDir() == Type::DK_Out) { outSels.push_back(wSel); }
  }
...
std::vector<CoreIR::Instance*> extractSubcircuit(
  ...
  for (auto portR : def->sel("self")->getSelects()) {
    ...
    if (!elem(port, determined)) { undetermined.insert(port); }
  }

  cout << "Determined ports" << endl;
  for (auto det : determined) { cout << "\t" << det->toString() << endl; }

  cout << "Undetermined ports" << endl;
  for (auto det : undetermined) { cout << "\t" << det->toString() << endl; }

  set<Instance*> alreadyAdded;

  set<Instance*> notAdded;
  for (auto inst : def->getInstances()) { notAdded.insert(inst.second); }
...
void addSubcircuitModule(
  ...
  for (auto conn : srcDef->getConnections()) {
    ...
    if ((newFst != nullptr) && (newSnd != nullptr)) {
...
void partiallyEvaluateCircuit(
  ...
  //   cout << "\t" << inst.second->toString() << " : " <<
  //   inst.second->getModuleRef()->toString() << endl;
  ...
  cout
    << "# of instances partially evaluated top after deleting dead instances = "
  ...
  cout << "# of instances partially evaluated top after constant folding = "
       << wholeTopMod->getDef()->getInstances().size() << endl;
}
...
Module* createSubCircuit(
  ...
  cout << "# of instances in subcircuit after deleting dead instances = "
       << topMod_conf->getDef()->getInstances().size() << endl;

  c->setTop(topMod_conf);
  c->runPasses({"removeconstduplicates"});

  cout << "# of instances in subcircuit after deleting duplicate constants = "
       << topMod_conf->getDef()->getInstances().size() << endl;
