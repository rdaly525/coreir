...
std::string cPrimitiveTypeString(Type& t) {
  ...
  if (isClkIn(t) || isNamedType(t, "clk")) { return "uint8_t"; }

  if (t.getKind() == Type::TK_BitIn) { return "uint8_t"; }

  if (t.getKind() == Type::TK_Bit) { return "uint8_t"; }

  if (isBitArrayOfLengthLEQ(t, 8)) { return "uint8_t"; }

  if (isBitArrayOfLengthLEQ(t, 16)) { return "uint16_t"; }

  if (isBitArrayOfLengthLEQ(t, 32)) { return "uint32_t"; }

  if (isBitArrayOfLengthLEQ(t, 64)) { return "uint64_t"; }
...
std::string cArrayTypeDecl(Type& t, const std::string& varName) {
  ...
  if (isClkIn(t) || isNamedType(t, "clk")) { return "uint8_t " + varName; }

  if (t.getKind() == Type::TK_BitIn) { return "uint8_t " + varName; }

  if (t.getKind() == Type::TK_Bit) { return "uint8_t " + varName; }

  if (isBitArrayOfLengthLEQ(t, 8)) { return "uint8_t " + varName; }

  if (isBitArrayOfLengthLEQ(t, 16)) { return "uint16_t " + varName; }

  if (isBitArrayOfLengthLEQ(t, 32)) { return "uint32_t " + varName; }

  if (isBitArrayOfLengthLEQ(t, 64)) { return "uint64_t " + varName; }
...
string maskMacroDef() {
  ...
  return "#define MASK(width, expr) " +
    parens(bitMaskString(width) + " & " + parens(expr)) + "\n\n";
}
