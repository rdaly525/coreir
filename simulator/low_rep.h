...
class LowUnop : public LowExpr {
 ...
 public:
  LowUnop(const std::string& op_, LowExpr* const op0_) : op(op_), op0(op0_) {}

  virtual std::string cString() const {
    if (op == "ULL") { return parens(parens(op0->cString()) + op); }
    return parens(op + op0->cString());
  }

  virtual ~LowUnop() { delete op0; }
};
...
class LowAssign : public LowStmt {
 ...
 public:
  LowAssign(LowExpr* const lhs_, LowExpr* const rhs_) : lhs(lhs_), rhs(rhs_) {}
...
class LowProgram {
 ...
 public:
  ...
  void addAssignStmt(LowExpr* const lhs, LowExpr* const rhs) {
