#include "ir/coreir.h"
#include "passes/analysis/createmodinstancemap.h"
#include "passes/transform/matchandreplace.h"
...
bool Passes::MatchAndReplace::runOnModule(Module* m) {
  ...
  for (auto cinst : cinstMap[pfirstKind]) {
    ...
    while (!work.empty()) {
      ...
      if (usedInstances.count(minst) > 0) { break; }

      // Check if the types are the same
      if (minst->getType() != pinst->getType()) { break; }
      ...
      for (auto lcons : this->inCons[idx]) {
        ...
        SelectPath otherPath = lcons.second[0].first;  // 0 because no NYI
                                                       // fanout
    ...
    if (numCompleted != instanceKey.size()) { continue; }

    // If user defined match function exists, check that.
    if (this->checkMatching) {
      if (!this->checkMatching(matchedInstances)) { continue; }
    }
  ...
  for (auto inst : usedInstances) { cdef->removeInstance(inst); }

  // Now inline all the passthrough Modules
  for (auto pt : passthroughsToInline) { inlineInstance(pt); }
