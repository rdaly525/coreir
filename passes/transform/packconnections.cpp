#include "passes/transform/packconnections.h"
  ...
  do {
    ...
    for (auto& conn : def->getConnections()) {
      ...
      if (isa<Select>(l) && isa<Select>(r)) {
        ...
        if (isNumber(ls->getSelStr()) && isNumber(rs->getSelStr())) {

          auto lsTp = cast<ArrayType>(ls->getParent()->getType());
          auto rsTp = cast<ArrayType>(rs->getParent()->getType());

          // cout << ls->getParent()->toString() << " len = " << lsTp->getLen()
          // << endl; cout << rs->getParent()->toString() << " len = " <<
          // rsTp->getLen() << endl;
    ...
    cout << "# of connections before deleting = "
         << def->getConnections().size() << endl;
    numOldConnections = def->getConnections().size();
    for (auto& gp : packs) {
      assert(gp.size() > 0);
      ...
      for (auto& conn : gp) { def->disconnect(conn.first, conn.second); }

      def->connect(parentL, parentR);
    }

    cout << "# of connections after deleting = " << def->getConnections().size()
         << endl;
    numNewConnections = def->getConnections().size();

  } while (numNewConnections < numOldConnections);
