...
// This is For a convenient macro to create the registerPass and deletePass
// functions
...
bool HelloModule::runOnModule(Module* m) {
  ...
  for (auto instmap : def->getInstances()) {
    ...
    // If the instance's InstantiableRef (the thing that it is instancing) is a
    // coreir.reg increment the counter
    if (inst->getInstantiableRef() == coreir_reg) { regInsts.push_back(inst); }
  }
  // Add this current module to the user datastructure we defined before
  if (regInsts.size() > 0) { registerMap[m] = regInsts; }
  return false;
}
...
int HelloModule::getTotalRegisters() {
  int total = 0;
  for (auto rmap : registerMap) { total += rmap.second.size(); }
  return total;
}
