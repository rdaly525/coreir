 ...
 * InstanceGraph Passes run on InstanceGraphNodes in topologically sorted order
 * (starting at leaves). an InstanceGraphNode represents an instantaible + it's
 * use-def structure.
 *
 * InstanceGraph Passes can make changes to the current Instantiable, or any
 * InstanceGraph which contains instances of it. There is an API for accessing
 * these instances as part of a sort of use-def tree.
 *
 * Another cool API this node supports is changing the Type of the current
 * Instantiable you are in. This can be dangerous, so lets see how to do this
 * safely.
 *
 * Lets make a pass that creates a global enable and wires it up to all the
 * registers
 */
...
// I made a copy of the hellmodule and embedded in the system. For external
// passes referencing analysis passes, you need to add it to the full build.
...
class HelloInstanceGraph : public InstanceGraphPass {
  ...
  // Lets define a name for our global_en. We will add this port to all modules
  // needing it
 ...
 public:
  ...
  // The PassManager will guarantee that all dependent passes will be valid
  // exactly before The current pass is run.
  void setAnalysisInfo() override { addDependency("hellomodule2"); }
};
...
bool HelloInstanceGraph::runOnInstanceGraphNode(InstanceGraphNode& node) {
  ...
  // Use getAnalysisPass template function to get access to the HelloModule
  // pass. Remember this will be guaranteed to run before your custom pass given
  // the setAnalysisInfo
  Passes::HelloModule* hellomodule = this
                                       ->getAnalysisPass<Passes::HelloModule>();
  ...
  if (hellomodule->hasRegisterInstances(i)) {
    ...
    for (auto reginst : hellomodule->getRegisterInstances(i)) {
      ...
      if (cast<RecordType>(reginst->getType())->getRecord().count("en")) {
        ...
        Instance* andInst = def->addInstance(
          "and" + c->getUnique(),
          coreir_and,
          {{"width", c->argInt(1)}});
  ...
  for (auto instmap : def->getInstances()) {
    ...
    if (cast<RecordType>(inst->getType())->getRecord().count(globalEnName)) {
      ...
      ASSERT(
        globalEn->getConnectedWireables().size() == 0,
        "gloalEn is already connected!");
  ...
  if (toWire.size() > 0) {
    ...
    for (auto w : toWire) { def->connect(w, selfGlobalEn); }
  }
...
// This is the macro that will define the registerPass and deletePass functions
// for you.
