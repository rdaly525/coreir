...
static inline std::string getInstanceName(Instance& w) {
...
static inline bool arrayAccess(Select* in) { return isNumber(in->getSelStr()); }

static inline bool isSelect(CoreIR::Wireable* fst) { return isa<Select>(fst); }
...
static inline bool isRegisterInstance(CoreIR::Wireable* fst) {
  ...
  // if (genRef == nullptr) {
  //  //std::cout << "ERROR: genRef is null for " << fst->toString() <<
  //  std::endl;

  //  Module* modRef = inst->getModuleRef();

  //  //std::cout << "Module ref is null ? " << (modRef == nullptr) <<
  //  std::endl;
...
static inline bool isLinebufferMemInstance(CoreIR::Wireable* fst) {
  ...
  return cast<Instance>(fst)->getModuleRef()->getRefName() ==
    "memory.rowbuffer";
...
std::unordered_map<std::string, CoreIR::Wireable*> getOutputSelects(
  CoreIR::Wireable* inst);

std::unordered_map<std::string, CoreIR::Wireable*> getInputSelects(
  CoreIR::Wireable* inst);
...
std::unordered_map<std::string, CoreIR::Type*> outputs(CoreIR::Module& mod);
