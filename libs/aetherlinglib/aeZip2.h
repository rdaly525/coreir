...
void Aetherling_createZipGenerator(Context* c) {
   ...
   * operator - the operator to parallelize. Note that it must have one input
   * known as "in" and one output known as "out"
   */
  Params zip2params = Params({//{"width", c->Int()},
                              {"numInputs", c->Int()},
  ...
  aetherlinglib->newTypeGen(
    ...
    [](Context* c, Values genargs) {  // Function to compute type
      ...
      return c->Record({{"in0", input0Type->Arr(numInputs)},
                        {"in1", input1Type->Arr(numInputs)},
                        {"out",
                         c->Record({{"el0", input0Type->getFlipped()},
  ...
  zip2->setGeneratorDefFromFun([](Context* c, Values genargs, ModuleDef* def) {
    ...
    assert(input0TypeRecord != 0); // 0 if cast failed, so input type wasn't
    RecordType RecordType* input1TypeRecord =
    dynamic_cast<RecordType*>(input1Type); assert(input1TypeRecord != 0); // 0
    if cast failed, so input type wasn't RecordType
    */
...
Module* Aetherling_convert2InputModuleTo2ZippedInput(
  ...
  Type* twoInZippedOneOutGenType = c->Record(
    {{"in",
      c->Record({{"el0", moduleToWrap->getType()->sel("in0")},
