...
uint getFlattenedSize(Context* c, Type* containerType, Type* innerType) {
  string errorString =
    "The type you are trying to flatten from doesn't contain the type you are "
    "trying to flatten to, container: " +
    containerType->toString() + ", inner: " + innerType->toString();
  ...
  for (
    ...
    Type* curType = outContainerType; curType != innerType;
    curType = curAsArray->getElemType()) {
    // If this cast fails, its probably because containerType wasn't an array
    // (to some degree) of innerType
    if (curType->isBaseType()) {
      cerr << errorString << endl;
      assert(
        !curType
           ->isBaseType());  //"The type you are trying to flatten from doesn't
                             // contain the type you are trying to flatten to");
    }
...
void Aetherling_createFlattenGenerator(Context* c) {
   ...
   * outputType - what you want to flatten it into, inputType must be some set
   * of arrays of this
   */
  ...
  aetherlinglib->newTypeGen(
    ...
    [](Context* c, Values genargs) {  // Function to compute type
      ...
      Type* singleElementOutputType = genargs.at("singleElementOutputType")
                                        ->get<Type*>();
      uint outputSize = getFlattenedSize(c, inputType, singleElementOutputType);
      return c->Record(
        {{"in", inputType}, {"out", singleElementOutputType->Arr(outputSize)}});
    });
  ...
  flatten->setGeneratorDefFromFun(
    [](Context* c, Values genargs, ModuleDef* def) {
      ...
      ArrayType* inputType = dyn_cast<ArrayType>(
        genargs.at("inputType")->get<Type*>());
      Type* singleElementOutputType = genargs.at("singleElementOutputType")
                                        ->get<Type*>();
      ArrayType* outputType = dyn_cast<ArrayType>(
        def->sel("self.out")->getType());
      // recursion:
      // base case: the input type is same as the full output type (array, not
      // single element) in this case, just iterate through all the inputs and
      // flatten them
      ...
      // recursion case:
      // for each element in top level array of the inputType, create a
      // flattener and then wire each of its outputs to this flattener's output
      else {
        ...
        for (uint i = 0; i < inputTypeLen; i++) {
          ...
          def->addInstance(
            "flattenInner_" + iStr,
            "aetherlinglib.flatten",
            {{"inputType", Const::make(c, inputType->getElemType())},
             {"singleElementOutputType",
              Const::make(c, singleElementOutputType)}});
