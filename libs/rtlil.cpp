  ...
  for (auto& name : rtlilShifts) {
    ...
    std::function<void(Context*, Values, ModuleDef*)> genFun =
      [name](Context* c, Values args, ModuleDef* def) {
        ...
        ASSERT(
          y_width >= a_width,
          "Shift operations must have output at least as long as bit vector "
          "being shifted");
  ...
  for (auto& name : rtlilBitwise) {
    ...
    std::function<void(Context*, Values, ModuleDef*)> genFun =
      [name](Context* c, Values args, ModuleDef* def) {
        ...
        ASSERT(
          y_width >= a_width,
          "Bitwise and arithmetic operations must have output at least as long "
          "as operands");
        ASSERT(
          y_width >= b_width,
          "Bitwise and arithmetic operations must have output at least as long "
